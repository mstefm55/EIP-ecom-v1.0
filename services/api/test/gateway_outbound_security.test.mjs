...
test("outbound timeout remains active while a successful response body is still streaming", async (t) => {
  const server = http.createServer((_req, res) => {
    ...
    res.write("partial");
    setTimeout(() => res.end("-late"), 6000);
  });
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
  t.after(() => new Promise((resolve) => server.close(resolve)));
  const address = server.address();

  // Aggregate security tests run files concurrently on shared CI runners. Give
  // response headers ample scheduling headroom while keeping the timeout well
  // before the delayed body completes, which is the security invariant here.
