...
test("payment routes and storefront integration expose governed checkout sessions without raw card collection", () => {
  ...
  assert.match(samaraApp, /PAYPAL_CHECKOUT_WINDOW_NAME = "samara-paypal-checkout"/);
  assert.match(samaraApp, /window\.open\([\s\S]*PAYPAL_CHECKOUT_WINDOW_NAME/);
  assert.match(samaraApp, /window\.opener\.postMessage/);
  assert.match(samaraApp, /event\.origin !== window\.location\.origin/);
  assert.match(samaraApp, /event\.source !== paypalCheckoutWindowRef\.current/);
  assert.match(samaraApp, /checkPaymentLifecycle\(paymentCode\)/);
  assert.match(samaraApp, /paypalCheckoutWindowRef\.current\.close\(\)/);
  assert.match(samaraApp, /window\.close\(\)/);
  assert.match(samaraApp, /You may now close this window\./);
  ...
  assert.match(samaraApp, /Payment successful/);
