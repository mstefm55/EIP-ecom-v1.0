...
export default async function ecomRoutes(app) {
  app.get("/perfect-fit/capability", async (req, reply) => {
    ...
    const { tenant_id: tenantId, identity_id: identityId } = sessionResult.session;
    const [canRead, canWrite] = await Promise.all([
      hasPermission(app, tenantId, identityId, "ECOM_PRODUCT_READ"),
      hasPermission(app, tenantId, identityId, "ECOM_PRODUCT_WRITE")
    ]);
  ...
  app.get(
    "/products/:id/perfect-fit",
    {
      schema: {
        params: { type: "object", required: ["id"], properties: { id: { type: "string", minLength: 36, maxLength: 36 } } }
      }
    },
    async (req, reply) => {
    ...
    "/products/:id/perfect-fit/link",
    {
      schema: {
        ...
        body: {
          ...
          required: ["perfect_fit"],
          properties: {
            perfect_fit: { type: "object" },
            shared_metadata: { type: "object" },
            origin: { type: "string", maxLength: 40 }
          }
        }
      }
    },
    ...
    "/perfect-fit/products/register",
    {
      schema: {
        body: {
          type: "object",
          additionalProperties: false,
          required: ["perfect_fit", "shared_metadata"],
          properties: {
            perfect_fit: { type: "object" },
            shared_metadata: { type: "object" }
          }
    ...
    "/products/:id/perfect-fit/sync",
    {
      schema: {
        ...
        body: {
          ...
          required: ["source"],
          properties: {
            source: { type: "string", enum: ["EIP", "PERFECT_FIT"] },
            perfect_fit_shared_metadata: { type: "object" },
