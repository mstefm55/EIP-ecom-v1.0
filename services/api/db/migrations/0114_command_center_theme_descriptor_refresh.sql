-- 0114_command_center_theme_descriptor_refresh.sql
-- Metadata-only refresh for environments where 0113 was already applied.
-- Keeps the Command Center descriptor governed and adds semantic theme tokens.
...
BEGIN
  ...
  LOOP
    ...
    SET tree=jsonb_set(surface_record.tree, '{children}', root_children, true),
        attrs=COALESCE(attrs, '{}'::jsonb) || '{"module":"dashboard","command_center_surface":true,"command_center_theme_tokens":true}'::jsonb,
        updated_at=now()
