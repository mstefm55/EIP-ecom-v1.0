...
Recent events stay compact in the main list. Select an event to open the viewport-centered details modal with event identifiers, tenant/actor/target context, connection/suffix context, reason, outcome, severity, and redacted metadata JSON. The page behind the modal is scroll-locked while event details are open. Copy actions are available for the event id, event type, and redacted JSON payload.
