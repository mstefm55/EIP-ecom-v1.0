...
| Admin DB explorer sensitive table masking and sensitive-token gate | OWASP ASVS 5.0 sensitive data protection; OWASP API3:2023 | implemented | `services/api/src/routes/admin_db_explorer.js`, `services/api/test/admin_db_explorer_security.test.mjs` | medium | Closed in final V1 security closure sweep |
...
| Password policy alignment with NIST guidance | NIST SP 800-63B memorized secrets | implemented | `services/api/src/auth/password.js`, `services/api/src/routes/auth.js`, `services/api/test/password_lifecycle.test.mjs` | medium | Closed in final V1 security closure sweep |
...
| Malware scanning or content disarm for uploaded tenant assets | OWASP ASVS 5.0 file upload; OWASP API8:2023 | implemented | `services/api/src/lib/uploadSecurity.js`, `services/api/test/upload_security.test.mjs` | medium | V1 closure: inline blocking plus external-required quarantine hook before publish |
...
3. Continue periodic access reviews for owner/admin roles, admin portfolio mappings, direct permissions, and tenant connection profiles.
4. Mature outbound provider allowlists beyond the current SSRF denylist as integrations become tenant-specific.
