...
function computeLinePricing({ material, quantity, currency, channel, jurisdiction, conditions }) {
  ...
  const baseCondition = selectCondition(priceConditions, ctx);
  ...
  if (unitPrice === null) {
    ...
    const preferredTier =
      tiers.find((tier) => normalizeUpper(tier?.currency) === normalizedCurrency) || tiers[0] || null;
    ...
    if (fallback !== null) unitPrice = fallback;
  }
...
async function buildQuote(app, tenantId, payload, context) {
  const conditions = await loadConditions(app, tenantId);
  const lines = [];
  let subtotal = 0;
  let discountTotal = 0;
  let taxTotal = 0;
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        client.release();
        return reply.code(500).send({ ok: false, error: "ORDER_CREATE_FAILED" });
      }
