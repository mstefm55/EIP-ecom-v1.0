...
function normalizeSlides(slides) {
  return Array.isArray(slides)
    ? slides
        ...
        .map((slide, index) => {
          const fit = String(slide.fit || slide.image_fit || "cover").toLowerCase() === "contain"
            ? "contain"
            : "cover";
          return {
      ...
      <div className="hero-slider-track" role="presentation">
        {slides.map((slide, index) => {
          ...
          return (
            ...
            >
              <div
                className={`hero-slide-overlay ${overlay}`}
                style={{
                  "--hero-overlay-strength": `${(slide.overlay_strength || 78) / 100}`,
                }}
              />
      ...
      {slides.length > 1 ? (
        <>
          <button type="button" className="hero-slider-nav is-prev" onClick={() => go(-1, true)} aria-label="Previous slide">
            <span className="hero-slider-nav-icon is-prev" aria-hidden="true" />
          </button>
          <button type="button" className="hero-slider-nav is-next" onClick={() => go(1, true)} aria-label="Next slide">
            <span className="hero-slider-nav-icon is-next" aria-hidden="true" />
          </button>
