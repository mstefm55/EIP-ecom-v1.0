...
const SHOW_CLASSIC_IMAGE_SIDE_BLUEPRINT_PANEL = true;
const SHOW_CLASSIC_IMAGE_BLUEPRINT_TAG = false;
const SHOW_CLASSIC_IMAGE_INFO_PANEL = false;
...
const getDifficultyInfo = (diff) => {
  const copyMap = localizeMetadataTree(
    perfectFitMetadata.componentUi.patternDifficulty.patternCard,
    'component.patternDifficulty.patternCard',
    pfUiT
  );
...
export default function PatternCard({
  ...
  const technicalSketchUrl =
    pattern.technicalSketchUrl ||
    pattern.technicalSketchAsset?.previewUrl ||
    pattern.technicalSketchAsset?.url ||
    '';

  const [format, setFormat] = useState('PDF');
  const [selectedSize, setSelectedSize] = useState(activeRecommendedSize || '8');
  const cardAvailableSizes = React.useMemo(() => {
    const sizes = Array.isArray(pattern.availableSizes) && pattern.availableSizes.length
      ? pattern.availableSizes
      : pattern.sizes;
    return Array.isArray(sizes) ? sizes.map((size) => String(size)) : [];
  }, [pattern.availableSizes, pattern.sizes]);
  ...
  useEffect(() => {
    setSelectedSize((current) => {
      if (cardAvailableSizes.includes(current)) return current;
      if (cardAvailableSizes.includes(activeRecommendedSize)) return activeRecommendedSize;
      return cardAvailableSizes[0] || activeRecommendedSize || '8';
    });
  }, [activeRecommendedSize, cardAvailableSizes]);
  ...
  return (
    <motion.div
      ...
      className={`w-full max-w-none bg-white rounded-[8px] md:rounded-[4px] border transition-all duration-300 overflow-hidden flex flex-row md:flex-col justify-between erp-pattern-card ${
        isActive
      ...
      data-presentation-source={pattern.presentationSource || 'legacy-commerce'}
      data-workspace-variant-id={pattern.workspaceVariantId || ''}
    ...
    >
      ...
      <div
        className="relative h-[168px] w-[116px] shrink-0 overflow-hidden border-r border-sand-150 group cursor-pointer sm:h-[184px] sm:w-[128px] md:h-auto md:w-full md:aspect-[3/4.2] md:border-r-0 md:border-b"
      ...
      >
        ...
        {viewMode === 'showcase' ? (
          ...
          <div
            className="absolute inset-x-0 bottom-0 top-[15%] hidden md:flex bg-stone-950/95 p-4 flex-col justify-between translate-y-full group-hover:translate-y-0 transition-all duration-300 ease-out z-20 text-stone-100 font-sans border-t border-white/10 backdrop-blur-md"
            onClick={(e) => {
          ...
          >
            <div className="space-y-3">
              <div className="flex justify-between items-center border-b border-white/15 pb-2">
                <span className="text-[10px] font-mono tracking-widest text-[#ba6446] uppercase font-bold flex items-center gap-1">
                  <Sparkles className="w-3 h-3 text-[#ba6446] animate-pulse" />{pfUiT("ui.components.patterncard.50d927b45b")}</span>
              ...
              <div className="space-y-2.5">
                ...
                <div className="space-y-1">
                  <span className="text-[9px] font-mono uppercase text-stone-300 tracking-wider flex items-center gap-1.5">
                    <Scissors className="w-3.5 h-3.5 text-stone-400" />{pfUiT("ui.components.patterncard.c94163a97c")}<strong className="text-white">{pattern.difficulty}</strong>
                  </span>
                ...
                <div className="space-y-1">
                  <span className="text-[9px] font-mono uppercase text-stone-300 tracking-wider flex items-center gap-1.5">
                    <Layers className="w-3.5 h-3.5 text-stone-400" />{pfUiT("ui.components.patterncard.c1a2e98d06")}</span>
                  <p className="text-[10.5px] text-stone-300 leading-relaxed font-sans line-clamp-2">
                ...
                <div className="space-y-1">
                  <span className="text-[9px] font-mono uppercase text-stone-300 tracking-wider flex items-center gap-1.5">
                    <Sliders className="w-3.5 h-3.5 text-stone-400" />{pfUiT("ui.components.patterncard.486973d0f9")}</span>
            ...
            <div className="pt-2.5 border-t border-white/10 flex items-center justify-between gap-2">
              <button
                ...
                type="button"
              >
                <Eye className="w-3 h-3" />{pfUiT("ui.components.patterncard.c5d5abe72a")}</button>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onQuickView(pattern);
              ...
              >{pfUiT("ui.components.patterncard.c5d5abe72a")}<ArrowRight className="w-3.5 h-3.5" />
              </button>
            </div>
          </div>
        ) : (
          /* Hover Quick View overlay */
          <div className="absolute inset-0 hidden md:flex bg-bark-950/20 opacity-0 group-hover:opacity-100 transition-all duration-300 items-center justify-center z-20">
            <button
              onClick={(e) => {
                e.stopPropagation();
                onQuickView(pattern);
            ...
            >
              ...
              <span>{pfUiT("ui.components.patterncard.c5d5abe72a")}</span>
            </button>
          </div>
        )}
        ...
        <button
          ...
          className={`absolute top-2 right-2 md:top-3 md:right-3 z-25 p-1.5 md:p-2 rounded-full backdrop-blur-md transition-all border shadow-xs hover:scale-110 active:scale-95 cursor-pointer ${
            isFavorite
        ...
        {SHOW_CLASSIC_IMAGE_INFO_PANEL && viewMode === 'grid' && (
          <div
            className="absolute left-3 top-3 z-15 max-w-[68%] rounded-[8px] border border-white/55 bg-white/68 px-3 py-2 text-bark-900 shadow-[0_8px_24px_rgba(32,24,19,0.12)] backdrop-blur-md"
            id={`classic-card-image-info-${pattern.id}`}
          >
            <span className="block font-serif text-[15px] font-semibold leading-none tracking-tight">
              Perfect Fit
            </span>
            <span className="mt-1 block truncate text-[9.5px] font-bold uppercase tracking-[0.18em] text-bark-600">
              {pattern.category.replace(/s$/, '')}
            </span>
            <div className="mt-2 flex flex-wrap items-center gap-1.5">
              <span className="rounded-full border border-sand-200/80 bg-white/70 px-2 py-0.5 text-[9.5px] font-semibold text-bark-700">
                {pattern.difficulty}
              </span>
            </div>
          </div>
        ...
        {SHOW_CLASSIC_IMAGE_SIDE_BLUEPRINT_PANEL && viewMode === 'grid' && (
          <div
            className="absolute inset-y-3 left-3 hidden w-[38%] rounded-[10px] border border-white/45 bg-white/38 backdrop-blur-[2px] md:flex flex-col justify-between p-2.5 select-none transition-colors duration-300 z-10 shadow-[0_12px_32px_rgba(32,24,19,0.12)]"
          ...
          >
            ...
            <div>
              ...
              <div className="mt-2.5" id={`viki-card-category-block-${pattern.id}`}>
                <span
                  ...
                  contentEditable={true}
                  suppressContentEditableWarning={true}
                  className="text-[7px] font-sans font-semibold tracking-wider text-bark-600 uppercase block leading-none cursor-text focus:outline-none"
                >
                  {pattern.category.replace(/s$/, '')}
                </span>
                <h4
                  ...
                  contentEditable={true}
                  suppressContentEditableWarning={true}
                  className="text-[10px] font-sans font-black tracking-widest text-bark-900 uppercase mt-0.5 leading-tight truncate cursor-text focus:outline-none"
                >
            ...
            <div
              ...
              className="h-28 w-full flex flex-col items-center justify-center py-1 opacity-95 hover:opacity-100 transition-all cursor-pointer relative group/flat rounded-[6px] hover:bg-white/20"
              id={`viki-card-flats-${pattern.id}`}
              title={pfUiT("ui.components.patterncard.046c2325cf")}
            >
              {technicalSketchUrl ? (
                <img
                  src={technicalSketchUrl}
                  alt=""
              ...
              <div className="absolute inset-0 bg-bark-950/5 opacity-0 group-hover/flat:opacity-100 transition-opacity flex items-center justify-center rounded-[4px]">
                <span className="text-[7.5px] font-mono uppercase bg-white/95 border border-sand-200 text-bark-800 px-2 py-1 rounded-[3px] shadow-md tracking-wider font-bold flex items-center gap-1 active:scale-95">
                  <Sparkles className="w-2.5 h-2.5 text-clay-600 animate-pulse" />{pfUiT("ui.components.patterncard.ed0d29028a")}</span>
              </div>
            </div>

            {/* Bottom: Sizes, Difficulty dots & level */}
            <div className="space-y-1.5 pt-1.5 border-t border-white/45" id={`viki-card-info-footer-${pattern.id}`}>
              <div>
                <span
                  id={`viki-card-sizes-label-${pattern.id}`}
                  contentEditable={true}
                  suppressContentEditableWarning={true}
                  className="text-[6px] font-sans font-semibold tracking-wider text-bark-600 uppercase block leading-none cursor-text focus:outline-none"
                >
                  SIZES
                </span>
                <span
                  ...
                  contentEditable={true}
                  suppressContentEditableWarning={true}
                  className="text-[8px] font-mono font-semibold text-bark-850 leading-none cursor-text focus:outline-none"
                >
                  0 - 22
                </span>
              </div>
              ...
              <div className="space-y-0.5">
                ...
                <span
                  ...
                  contentEditable={true}
                  suppressContentEditableWarning={true}
                  className="text-[7px] font-sans font-bold tracking-wider text-bark-800 uppercase block leading-none cursor-text focus:outline-none"
                >
                  {pattern.difficulty}
                </span>
              </div>
        ...
        {SHOW_CLASSIC_IMAGE_BLUEPRINT_TAG && (
        <div className="absolute bottom-2 right-2 bg-white/95 backdrop-blur-xs px-1.5 py-0.5 rounded-[4px] border border-sand-150 flex items-center justify-between shadow-3xs z-10" id={`pdf-banner-${pattern.id}`}>
          ...
          >{pfUiT("ui.components.patterncard.0289293dd8")}</span>
        </div>
        )}
      </div>

      {/* Details Box */}
      <div className="min-w-0 flex-1 p-3 md:p-4 flex flex-col justify-between bg-[#fdfdfc]" id="details-box">
        <div>
          {/* Viki Sews Inspired lowercase/titlecase display name and price first */}
          <div className="flex flex-col mb-1.5 md:mb-2.5" id="viki-card-header">
            <div className="flex items-center justify-between" id="price-rating-row">
              <span
              className="font-serif text-base md:text-lg font-medium text-bark-900 erp-pattern-price"
                id="viki-price-tag"
              ...
              {ratingStats ? (
                <button
                  ...
                  title={pfUiT("ui.components.patterncard.3293398870")}
                >
                ...
                >
                  ...
                  <span className="hidden sm:inline leading-none">Reviews &amp; Gallery</span>
                </button>
              )}
            </div>
            <span
              onClick={onSelect}
              className="mt-1 block line-clamp-2 cursor-pointer font-serif text-[15px] md:text-[17px] font-semibold leading-tight tracking-tight text-bark-950 transition-colors hover:text-clay-600 erp-pattern-title"
              id="viki-title-tag"
              data-erp-field="name"
            >
              {pattern.name}
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-1.5 md:hidden">
              <span className="rounded-full border border-sand-200 bg-sand-50 px-2 py-0.5 text-[8px] font-semibold uppercase tracking-wide text-bark-600">
                {pattern.category}
              </span>
              <span className="rounded-full border border-sand-200 bg-white px-2 py-0.5 text-[8px] font-semibold uppercase tracking-wide text-bark-600">
                {pattern.difficulty}
              </span>
              {cardAvailableSizes.length > 0 && (
                <span className="text-[8px] font-mono text-bark-400">
                  {cardAvailableSizes[0]}–{cardAvailableSizes[cardAvailableSizes.length - 1]}
                </span>
              )}
            </div>
        ...
        <div className="mt-1.5 md:mt-2" id={`pattern-card-actions-${pattern.id}`}>
          <div className="flex gap-1.5 items-center" id={`actions-row-${pattern.id}`}>
            ...
            <motion.button
              ...
              disabled={justAdded}
              className={`flex-1 py-2 md:py-2.5 rounded-[4px] font-bold text-[9px] md:text-xs uppercase tracking-[0.08em] md:tracking-widest transition-all duration-300 flex items-center justify-center gap-1 md:gap-1.5 cursor-pointer relative overflow-hidden ${
                justAdded
              ...
              <AnimatePresence mode="wait">
                {justAdded ? (
                  ...
                  >
                    ...
                    <span className="font-serif italic font-medium tracking-wide">{pfUiT("ui.components.patterncard.6950cf8321")}</span>
                  </motion.div>
                ) : (
                  <motion.div
                    key="idle"
                  ...
                  >
                    ...
                    <span className="md:hidden">Add — ${(typeof currentPrice === 'number' && !isNaN(currentPrice) ? currentPrice : 0).toFixed(0)}</span>
                    <span className="hidden md:inline">Add to Cart — ${(typeof currentPrice === 'number' && !isNaN(currentPrice) ? currentPrice : 0).toFixed(0)}</span>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <button
              onClick={onSelect}
              className="px-2 py-2 md:px-2.5 md:py-2.5 bg-white hover:bg-sand-50 border border-sand-250/70 text-bark-800 hover:text-clay-605 rounded-[4px] flex items-center justify-center cursor-pointer transition-colors active:scale-95 shadow-3xs"
              title={pfUiT("ui.components.patterncard.d074b8cfc5")}
              type="button"
              ...
              <div className="p-4 sm:p-5 border-b border-sand-200 bg-white flex items-center justify-between flex-shrink-0">
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-[9px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 bg-clay-50 border border-clay-200/50 text-clay-700 rounded-[4px]">{pfUiT("ui.components.patterncard.4b70dad3c5")}</span>
                ...
                <button
                  ...
                  className="p-1.5 text-bark-400 hover:text-bark-800 transition-colors rounded-full hover:bg-sand-100 cursor-pointer"
                  title={pfUiT("ui.components.patterncard.cbaf01251c")}
              ...
              <div className="flex-1 overflow-y-auto flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-sand-200" id="video-modal-body">
                ...
                <div className="flex-1 p-4 sm:p-5 flex flex-col gap-4 bg-white" id="player-column">
                  ...
                  <div className="flex border-b border-sand-150 text-xs font-semibold flex-shrink-0" id="player-tabs">
                    ...
                    >{pfUiT("ui.components.patterncard.992b256449")}</button>
                    <button
                      onClick={() => {
                    ...
                    >{pfUiT("ui.components.patterncard.9ffe354b0b")}</button>
                  </div>
                        ...
                        <div className="z-10 space-y-2.5 flex-shrink-0" id="sim-bottom-bar">
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between text-[9px] font-mono text-sand-300">
                              <span>{pfUiT("ui.components.patterncard.1e64d1414d")}</span>
                          ...
                          <div className="flex items-center justify-center gap-2.5">
                            <button
                              ...
                              title={pfUiT("ui.components.patterncard.6f3d6a54ab")}
                            >
                            ...
                            >
                              {isPlayingSimulator ? (
                                <>
                                  ...
                                  <span>{pfUiT("ui.components.patterncard.750cdbd4bc")}</span>
                                </>
                              ) : (
                                <>
                                  <PlayCircle className="w-4 h-4" />
                                  <span>{pfUiT("ui.components.patterncard.52e78c7bcf")}</span>
                                </>
                              )}
                            </button>
                          </div>
                  ...
                  <div className="p-3 bg-sand-100/50 border border-sand-200/50 rounded-lg space-y-1" id="active-step-desc">
                    <span className="text-[9px] font-mono font-bold text-bark-400 uppercase tracking-widest">{pfUiT("ui.components.patterncard.94b993fbc8")}</span>
                ...
                <div className="w-full lg:w-76 p-4 sm:p-5 flex flex-col gap-4 overflow-y-auto bg-[#FAF8F5]" id="curriculum-column">
                  ...
                  <div className="p-2.5 bg-white border border-sand-200 rounded-lg flex items-center gap-2.5 shadow-3xs flex-shrink-0" id="instructor-card">
                    ...
                    <div className="space-y-0.5">
                      <span className="text-[7.5px] font-mono uppercase tracking-wider text-bark-400 block leading-none font-bold">{pfUiT("ui.components.patterncard.0f729d8570")}</span>
                      <h4 className="text-[11px] font-bold text-bark-900 leading-none">{pattern.tutorial?.instructor || "Perfect Fit Master"}</h4>
                      <p className="text-[9px] text-bark-500 leading-none">{pfUiT("ui.components.patterncard.0467c9aa80")}</p>
                    </div>
                  </div>

                  {/* Steps Scroller */}
                  <div className="space-y-2 flex-grow" id="steps-panel">
                    <h4 className="text-[9px] font-mono uppercase font-bold text-bark-400 tracking-wider">{pfUiT("ui.components.patterncard.0387477596")}</h4>
                  ...
                  <div className="p-3 bg-amber-50/50 border border-amber-200/50 rounded-lg space-y-1.5 text-xs flex-shrink-0" id="pro-tips-card">
                    <h4 className="font-bold text-amber-900 flex items-center gap-1 text-[9px] font-mono uppercase tracking-wider">
                      <BookOpen className="w-3 h-3 text-amber-700" />{pfUiT("ui.components.patterncard.d49ba053c9")}</h4>
            ...
            >
              ...
              <div className="p-4 sm:p-5 border-b border-sand-200 bg-white flex items-center justify-between flex-shrink-0">
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-[9px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 bg-clay-50 border border-clay-200/50 text-clay-700 rounded-[4px] flex items-center gap-1">
                      <Cpu className="w-2.5 h-2.5 animate-pulse" />{pfUiT("ui.components.patterncard.c3c99b1cee")}</span>
                    <span className="text-[9px] font-mono text-bark-450 uppercase tracking-wider">{pfUiT("ui.components.patterncard.78a1981510")}</span>
                  </div>
                  <h3 className="font-serif text-base sm:text-lg font-semibold text-bark-950 flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1.5 text-bark-400 hover:text-bark-800 transition-colors rounded-full hover:bg-sand-100 cursor-pointer"
                  title={pfUiT("ui.components.patterncard.1ad2ad7e4b")}
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-6" id="swatch-modal-body">
                ...
                <div className="flex flex-col gap-4 bg-white p-4 rounded-xl border border-sand-200" id="tech-drawing-panel">
                  <div className="flex items-center justify-between flex-shrink-0">
                    <div className="space-y-0.5">
                      <h4 className="text-[10.5px] font-mono uppercase tracking-widest text-bark-450 font-bold">{pfUiT("ui.components.patterncard.90824d1771")}</h4>
                  ...
                  >
                    ...
                    <div className="absolute bottom-3 left-3 bg-white/90 border border-sand-200/80 rounded px-2 py-1 flex items-center gap-1.5 shadow-2xs backdrop-blur-sm z-20">
                      ...
                      <span className="text-[7.5px] font-mono uppercase tracking-wider text-bark-700 font-bold">{pfUiT("ui.components.patterncard.4a9ea78c97")}</span>
                    </div>
                  </div>
                  ...
                  <div className="p-3 bg-sand-50 border border-sand-200/80 rounded-lg space-y-2.5 text-xs flex-grow" id="tech-drawing-specs">
                    <div>
                      <span className="text-[8px] font-mono uppercase tracking-widest text-bark-400 block font-bold">{pfUiT("ui.components.patterncard.b42ce8981f")}</span>
                      <p className="text-[10.5px] font-sans font-medium text-bark-700 mt-1">{pfUiT("ui.components.patterncard.8c7bca8cc8")}<span className="font-bold text-clay-700">{pfUiT("ui.components.patterncard.2b9d3b367f")}</span> or standard overcast finish. 3/8 in (1cm) seam allowance included in blueprint pattern pages.
                      </p>
                    </div>

                    {pattern.features && pattern.features.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-[8px] font-mono uppercase tracking-widest text-bark-400 block font-bold">{pfUiT("ui.components.patterncard.0cd235601a")}</span>
                    ...
                    <div className="pt-2 border-t border-sand-200/60 flex items-center justify-between text-[9px] font-mono text-bark-450">
                      ...
                      <span>{pfUiT("ui.components.patterncard.453e09a296")}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-4 bg-white p-4 rounded-xl border border-sand-200" id="generative-textile-panel">
                  <div className="space-y-0.5 flex-shrink-0">
                    <h4 className="text-[10.5px] font-mono uppercase tracking-widest text-bark-450 font-bold">{pfUiT("ui.components.patterncard.8f23ae4d20")}</h4>
                    <p className="text-[11.5px] font-serif italic text-bark-800 font-semibold">{pfUiT("ui.components.patterncard.015f169d4f")}</p>
                  </div>
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between gap-3 pt-1">
                            <div className="flex items-center gap-2.5 min-w-0">
                              ...
                              <div className="w-9 h-12 bg-white rounded border border-sand-250 flex flex-col justify-between overflow-hidden shadow-3xs p-0.5 flex-shrink-0">
                                ...
                                <div className="text-[4px] font-mono leading-none pt-0.5 text-center font-bold text-bark-900 tracking-tighter uppercase">{pfUiT("ui.components.patterncard.7fa379246f")}</div>
                              </div>
                              <div className="min-w-0">
                            ...
                            >
                              <span>{pfUiT("ui.components.patterncard.16cd44c327")}</span>
                          ...
                          >
                            <div className="flex justify-between items-center">
                              ...
                              {isHoveredState && (
                                <span className="text-[7.5px] font-mono text-clay-600 bg-clay-50 px-1 rounded animate-pulse">{pfUiT("ui.components.patterncard.519ddee2c3")}</span>
                              )}
                            </div>
                          ...
                          <div className="text-[7.5px] font-mono text-bark-400 border-t border-sand-200/20 pt-1.5 flex justify-between items-center">
                            ...
                            <span>{pfUiT("ui.components.patterncard.13347604cc")}</span>
                          </div>
                        </motion.div>
                      );
                    })()}
                  ...
                  <div className="space-y-1.5 flex-shrink-0" id="ai-generator-panel">
                    <span className="text-[8px] font-mono uppercase tracking-widest text-bark-400 block font-bold">{pfUiT("ui.components.patterncard.87f322d653")}</span>
                    <div className="flex gap-2">
                      <div className="relative flex-grow">
                        <input
                          ...
                          placeholder={pfUiT("ui.components.patterncard.8eb126cbbd")}
                      ...
                      >
                        ...
                        <span>{pfUiT("ui.components.patterncard.3c9508c96a")}</span>
                      </button>
                    </div>
                  </div>
                        ...
                        <div className="absolute inset-0 bg-bark-950/90 text-emerald-400 font-mono text-[8px] p-4 flex flex-col justify-between z-40 select-none animate-[pulse_3s_infinite]">
                          <div className="space-y-1">
                            <div className="flex items-center gap-1.5 text-white font-bold text-[9px] border-b border-emerald-500/20 pb-1 mb-1.5">
                              ...
                              <span>{pfUiT("ui.components.patterncard.0e096dd85a")}</span>
                            </div>
                          ...
                          <div className="flex items-center justify-between border-t border-emerald-500/20 pt-1 text-[7.5px] text-emerald-500">
                            <span>{pfUiT("ui.components.patterncard.4d740eacc6")}</span>
                            <span>{pfUiT("ui.components.patterncard.c76df9fd04")}</span>
                          </div>
                        </div>
                      ) : (
                        ...
                        >
                          ...
                          {isMagnifying && (
                            ...
                            >
                              ...
                              <div className="absolute top-1 right-3 text-[5.5px] font-mono text-[#ba6446] bg-white/70 px-1 py-0.2 rounded font-bold">{pfUiT("ui.components.patterncard.f9e8aa2d15")}</div>
                            </div>
                          )}
