...
const INITIAL_STATE = {
  open: false,
  file: null,
  title: pfUiT("ui.components.imageassetstudiomodal.imageStudio", {}, "Image studio"),
...
export default function ImageAssetStudioModal({
  ...
  const imageStudioUi = perfectFitMetadata.componentUi.imageAssetStudio;
  const SIZE_PRESETS = localizeMetadataTree(imageStudioUi.sizePresets, 'component.imageAssetStudio.sizePresets', pfUiT);
  const FORMAT_OPTIONS = localizeMetadataTree(imageStudioUi.formatOptions, 'component.imageAssetStudio.formatOptions', pfUiT);
  const DEFAULT_WORKFLOW_PROFILES = localizeMetadataTree(imageStudioUi.defaultWorkflowProfiles, 'component.imageAssetStudio.defaultWorkflowProfiles', pfUiT);
    ...
    <div className="image-studio-backdrop" onPointerMove={updateDrag} onPointerUp={endDrag}>
      <div className="image-studio-modal" onClick={(event) => event.stopPropagation()}>
        <header className="image-studio-header">
          <div>
            <p className="image-studio-label">{pfUiT("ui.components.imageassetstudiomodal.8156bf2e01")}</p>
            <h3>{title || "Image studio"}</h3>
            <p>{pfUiT("ui.components.imageassetstudiomodal.852b63c680")}</p>
          </div>
          <button type="button" className="image-studio-close" onClick={onCancel}>{pfUiT("ui.components.imageassetstudiomodal.5c35979535")}</button>
        </header>
            ...
            <div className="image-studio-canvas-wrap">
              ...
              >
                <img
                  ref={imgRef}
                  src={displaySource}
                  alt={pfUiT("ui.components.imageassetstudiomodal.0609bbddb7")}
              ...
              <p className="image-studio-help">{pfUiT("ui.components.imageassetstudiomodal.cd6675e9a4")}</p>
            </div>

            <aside className="image-studio-controls">
              <div className="image-studio-profile-stack">
                <p className="image-studio-profile-title">{pfUiT("ui.components.imageassetstudiomodal.9d99da495e")}</p>
              ...
              <div className="image-studio-crop-tool">
                <div className="image-studio-crop-head">
                  <p className="image-studio-crop-title">{pfUiT("ui.components.imageassetstudiomodal.dfe1412d96")}</p>
                  <div className="image-studio-crop-actions">
                    <button type="button" className="image-studio-toggle" onClick={fitCropToOutputRatio}>{pfUiT("ui.components.imageassetstudiomodal.2d2808da83")}</button>
                    <button type="button" className="image-studio-toggle" onClick={applyCropToSource} disabled={!imageReady || working}>{pfUiT("ui.components.imageassetstudiomodal.730a3995ed")}</button>
                  </div>
                </div>
                <div ref={cropStageRef} className="image-studio-crop-stage">
                  <img
                    src={displaySource}
                    alt={pfUiT("ui.components.imageassetstudiomodal.ada64bb196")}
              ...
              <div className="image-studio-control-grid">
                <label>{pfUiT("ui.components.imageassetstudiomodal.4819b067c3")}<select value={presetId} onChange={(event) => handlePresetChange(event.target.value)}>
                ...
                <label>{pfUiT("ui.components.imageassetstudiomodal.7c8ebe23d1")}<input
                    type="number"
                    min={1}
                    max={6000}
                    value={outputWidth}
                ...
                <label>{pfUiT("ui.components.imageassetstudiomodal.b35a1af5e6")}<input
                    type="number"
                    min={1}
                    max={6000}
                    value={outputHeight}
                ...
                <label>{pfUiT("ui.components.imageassetstudiomodal.e671467c7d")}<select value={fitMode} onChange={(event) => setFitMode(event.target.value === "contain" ? "contain" : "cover")}>
                    <option value="cover">Cover (crop)</option>
                    <option value="contain">Contain (pad)</option>
                  </select>
                </label>
                <label>{pfUiT("ui.components.imageassetstudiomodal.2653fd1636")}<select value={mimeType} onChange={(event) => setMimeType(event.target.value)}>
                ...
                <label>{pfUiT("ui.components.imageassetstudiomodal.cf7ff75342")}<input
                    type="color"
              ...
              <div className="image-studio-slider-stack">
                <label>{pfUiT("ui.components.imageassetstudiomodal.d7e67b2f29")}<input
                    type="range"
                    min={0.4}
                    max={3.5}
                    step={0.01}
                ...
                <label>{pfUiT("ui.components.imageassetstudiomodal.6a88f09bae")}<input
                    type="range"
                    min={-180}
                    max={180}
                    step={1}
                ...
                <label>{pfUiT("ui.components.imageassetstudiomodal.09e9e4edb1")}<input
                    type="range"
                    min={40}
                    max={180}
                    step={1}
                ...
                <label>{pfUiT("ui.components.imageassetstudiomodal.595041c223")}<input
                    type="range"
                    min={40}
                    max={180}
                    step={1}
                ...
                <label>{pfUiT("ui.components.imageassetstudiomodal.2049ee17ba")}<input
                    type="range"
                    min={0}
                    max={200}
                    step={1}
                ...
                <label>{pfUiT("ui.components.imageassetstudiomodal.ff3a443680")}<input
                    type="range"
                    min={0}
                    max={8}
                    step={0.1}
                ...
                {mimeType !== "image/png" ? (
                  <label>{pfUiT("ui.components.imageassetstudiomodal.f1f32d7f7f")}<input
                      type="range"
                      min={45}
                      max={100}
                      step={1}
              ...
              <div className="image-studio-toggle-row">
                ...
                >{pfUiT("ui.components.imageassetstudiomodal.d66ed5036d")}</button>
                <button
                  type="button"
                  className={`image-studio-toggle${flipY ? " is-active" : ""}`}
                  onClick={() => setFlipY((prev) => !prev)}
                >{pfUiT("ui.components.imageassetstudiomodal.586a60ab4e")}</button>
                <button
                  type="button"
                  className="image-studio-toggle"
                  onClick={resetAdjustments}
                >{pfUiT("ui.components.imageassetstudiomodal.39eb99559e")}</button>
              </div>
            </aside>
          </div>
        ) : (
          <div className="image-studio-empty">{pfUiT("ui.components.imageassetstudiomodal.37a7c13168")}</div>
        )}
        ...
        <footer className="image-studio-footer">
          ...
          <div className="image-studio-actions">
            <button type="button" className="ghost" onClick={onCancel}>{pfUiT("ui.components.imageassetstudiomodal.3c715eebaa")}</button>
