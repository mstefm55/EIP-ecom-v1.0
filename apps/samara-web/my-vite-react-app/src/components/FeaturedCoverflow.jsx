...
export default function FeaturedCoverflow({
  ...
  const rel = (index) => {
    ...
    if (n <= 1) return 0;
    let d = index - active;
    // Even-sized carousels (especially 2 items) have an ambiguous midpoint.
    // Force midpoint direction to follow the last navigation direction to keep circular flow.
  ...
  return (
    <div className={`fcEmbed ${compact ? "is-compact" : ""} fcTheme--${theme}`}>
      <div className={`fcStage ${compact ? "fcStage--compact" : ""}`} ref={rootRef} aria-label={ariaLabel}>
        ...
        <button className="fcNav fcPrev" type="button" onClick={() => go(-1, true)} aria-label="Previous">
          &lt;
        </button>
        <button className="fcNav fcNext" type="button" onClick={() => go(1, true)} aria-label="Next">
          &gt;
        </button>
