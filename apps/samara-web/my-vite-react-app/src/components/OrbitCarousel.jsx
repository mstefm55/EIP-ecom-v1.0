...
export default function OrbitCarousel({
  ...
  // 
  // Math dimensions for the compressed orbit
const radiusX = isMobile ? 120 : 330;
const radiusY = isMobile ? 28 : 70;
const N = patterns.length;

// Compressed orbit settings.
// Smaller angle step = more overlap / tighter packing.
const ORBIT_ANGLE_STEP = isMobile ? 0.34 : 0.26;

// How many cards to show on each side of the active card.
const VISIBLE_SIDE_COUNT = isMobile ? 2 : 6;

// Card scaling.
const FRONT_SCALE = isMobile ? 1.02 : 1.18;
const SIDE_SCALE_DROP = isMobile ? 0.08 : 0.06;
  ...
  return (
    <div className="space-y-12" id="orbit-carousel-outer-wrapper">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-3" id="orbit-header-block">
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-serif text-bark-950 font-light tracking-tight leading-[1.12]" id="orbit-title">{pfUiT("ui.components.orbitcarousel.bd3627b3b8")}<span className="font-serif italic text-clay-600 font-normal">{pfUiT("ui.components.orbitcarousel.41abed0d1c")}</span>
        </h2>
        <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-serif italic text-bark-700 leading-relaxed" id="orbit-description">{pfUiT("ui.components.orbitcarousel.6a84bc96ff")}</p>
      </div>

      {/* THE 3D ORBIT STAGE */}
      <div
  className="relative h-[440px] md:h-[560px] lg:h-[620px] bg-transparent overflow-visible flex items-center justify-center"
...
>
        ...
        <div
  className="absolute left-1/2 top-[58%] -translate-x-1/2 w-[260px] md:w-[520px] h-[90px] md:h-[150px] bg-[radial-gradient(ellipse_at_center,rgba(45,30,21,0.12)_0%,rgba(45,30,21,0.06)_38%,transparent_72%)] rounded-full blur-2xl pointer-events-none"
  id="orbit-center-glow"
/>
...
const zIndex = isFront ? 50 : Math.round(10 + 20 * normalizedDepth);
const rotate = 0;
const isVisible = true;

// Visual depth tuning
const sideSeparationX = isFront ? 0 : x < 0 ? -42 : 42;
const verticalDepthOffset = isFront ? -82 : 34;
const visualOpacity = isFront ? 1 : Math.max(0.42, opacity * 0.78);
const visualFilter = isFront ? 'none' : 'saturate(0.82) brightness(0.86)';
return (            
              <div
                key={pattern.id}
                ...
                style={{
  transform: `translate3d(calc(-50% + ${x + sideSeparationX}px), calc(-50% + ${y + verticalDepthOffset - 45}px), 0) scale(${scale}) rotate(${rotate}deg)`,
zIndex: zIndex,
...
pointerEvents: isVisible ? 'auto' : 'none',
              ...
              >
                ...
                <div
                  className={`w-[170px] sm:w-[220px] md:w-[280px] lg:w-[310px] aspect-[3/4.2] bg-white rounded-[6px] overflow-hidden relative transition-all duration-700 flex flex-col justify-end ${
  isFront
    ? 'shadow-[0_32px_85px_rgba(45,30,21,0.22)] ring-1 ring-bark-300/50'
    : 'shadow-[0_12px_32px_rgba(45,30,21,0.08)] ring-1 ring-white/70 hover:shadow-[0_18px_42px_rgba(45,30,21,0.13)]'
}`}
        ...
        <div className="absolute left-4 right-4 flex justify-between pointer-events-none z-30" id="orbit-stage-controls">
          <button
            ...
            aria-label={pfUiT("ui.components.orbitcarousel.1d003d542e")}
            id="orbit-arrow-prev"
          >
          ...
          <button
            onClick={handlePrev}
            className="w-10 h-10 rounded-full bg-white/90 hover:bg-white border border-sand-200 text-bark-800 flex items-center justify-center transition-all hover:shadow-md cursor-pointer active:scale-95 pointer-events-auto"
            aria-label={pfUiT("ui.components.orbitcarousel.0acaa84962")}
            id="orbit-arrow-next"
          >
        ...
        <div
  className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-2.5 z-[60] rounded-full bg-[#FAF8F5]/85 px-3 py-2 shadow-[0_10px_30px_rgba(45,30,21,0.08)] backdrop-blur-sm"
  id="orbit-indicator-dots"
>
          {patterns.map((_, idx) => (
            <button
              key={idx}
