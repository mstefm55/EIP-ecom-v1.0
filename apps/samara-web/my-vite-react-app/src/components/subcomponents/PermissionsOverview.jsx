    ...
    <div className="bg-[#FAF9F6] border border-sand-200 rounded-2xl p-5 md:p-8 space-y-8 shadow-ux font-sans text-bark-800 animate-fadeIn" id="permissions-overview-container">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5 pb-5 border-b border-sand-200">
        <div className="space-y-1.5 text-left">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-clay-700 font-bold">{pfUiT("ui.components.subcomponents.permissionsoverview.542f3825e3")}</span>
          </div>
          <h3 className="text-2xl font-serif text-bark-950 font-semibold tracking-tight">{pfUiT("ui.components.subcomponents.permissionsoverview.0e70619934")}</h3>
          <p className="text-xs text-bark-600 leading-relaxed font-sans max-w-2xl">{pfUiT("ui.components.subcomponents.permissionsoverview.671d8615c2")}</p>
        </div>
        ...
        <div className="flex items-center bg-sand-100 p-1 rounded-xl border border-sand-200/60 self-start lg:self-center shrink-0 shadow-4xs">
          ...
          >
            ...
            <span>{pfUiT("ui.components.subcomponents.permissionsoverview.8c258b2486")}</span>
          </button>
          <button
          ...
          >
            ...
            <span>{pfUiT("ui.components.subcomponents.permissionsoverview.f587bb6ce8")}</span>
          </button>
          <button
          ...
          >
            ...
            <span>{pfUiT("ui.components.subcomponents.permissionsoverview.f1c2717fd9")}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4" id="permissions-stats-grid">
        <div className="bg-white border border-sand-200 p-4 rounded-xl flex items-center gap-3 shadow-4xs text-left">
          ...
          <div>
            <span className="block text-[10px] font-mono text-bark-450 uppercase font-semibold">{pfUiT("ui.components.subcomponents.permissionsoverview.377da2897b")}</span>
        ...
        <div className="bg-white border border-sand-200 p-4 rounded-xl flex items-center gap-3 shadow-4xs text-left">
          ...
          <div>
            <span className="block text-[10px] font-mono text-bark-450 uppercase font-semibold">{pfUiT("ui.components.subcomponents.permissionsoverview.bbcb7a240d")}</span>
        ...
        <div className="bg-white border border-sand-200 p-4 rounded-xl flex items-center gap-3 shadow-4xs text-left">
          ...
          <div>
            <span className="block text-[10px] font-mono text-bark-450 uppercase font-semibold">{pfUiT("ui.components.subcomponents.permissionsoverview.752f3f141f")}</span>
        ...
        <div className="bg-white border border-sand-200 p-4 rounded-xl flex items-center gap-3 shadow-4xs text-left col-span-2 md:col-span-1">
          ...
          <div className="overflow-hidden truncate">
            <span className="block text-[10px] font-mono text-bark-450 uppercase font-semibold truncate">{pfUiT("ui.components.subcomponents.permissionsoverview.bbb6ff9d11")}</span>
            <strong className="text-xs text-bark-900 font-mono block leading-tight truncate" title={meta.authStandard}>{pfUiT("ui.components.subcomponents.permissionsoverview.c69267cad2")}</strong>
          </div>
        </div>
      </div>
      ...
      {activeTab !== 'json' && (
        <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center bg-white p-4 border border-sand-200 rounded-xl shadow-4xs">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              type="text"
              placeholder={pfUiT("ui.components.subcomponents.permissionsoverview.1a126bf024")}
              value={searchTerm}
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            <label className="flex items-center gap-2 cursor-pointer bg-sand-50/60 hover:bg-sand-100/50 border border-sand-200/80 px-3.5 py-1.5 rounded-lg text-xs font-mono transition-all">
              ...
              />
              <span className="text-bark-700">{pfUiT("ui.components.subcomponents.permissionsoverview.a1a83c298c")}</span>
            ...
            <div className="text-[10px] text-bark-500 flex items-center gap-1.5">
              ...
              <span>{pfUiT("ui.components.subcomponents.permissionsoverview.a7a684be73")}<code className="bg-sand-100 px-1 py-0.2 rounded font-mono">{pfUiT("ui.components.subcomponents.permissionsoverview.20c1b24c2e")}</code>.</span>
            </div>
          ...
          >
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-sand-50 border-b border-sand-200 text-[10px] font-mono uppercase text-bark-500 tracking-wider">
                    <th className="p-4 pl-5 font-bold">{pfUiT("ui.components.subcomponents.permissionsoverview.4626077ccd")}</th>
                  ...
                  {filteredComponentKeys.length > 0 ? (
                    filteredComponentKeys.map(componentKey => {
                      const comp = ComponentRegistry[componentKey] || {
                        name: componentKey,
                        description: pfUiT(
                          'ui.permissions.component.unregistered',
                          {},
                          'Component not registered in workspace registry.'
                        ),
                        icon: HelpCircle
                      };
                            ...
                            return (
                              <td key={roleId} className="p-4 text-center">
                                <div className="flex flex-col items-center justify-center gap-1.5">
                                  ...
                                  {/* Override Marker */}
                                  {isOverride ? (
                                    <div className="flex items-center gap-1 bg-amber-50 text-amber-800 border border-amber-200/60 px-1.5 py-0.2 rounded font-mono text-[8.5px] font-bold animate-pulse" title={pfUiT("ui.components.subcomponents.permissionsoverview.2601024089")}>
                    ...
                    <tr>
                      <td colSpan={totalRolesCount + 1} className="p-8 text-center text-bark-450 space-y-2">
                        ...
                        <strong className="block text-bark-750 font-serif text-sm">{pfUiT("ui.components.subcomponents.permissionsoverview.87beb5525d")}</strong>
                        <p className="text-xs max-w-md mx-auto leading-relaxed">{pfUiT("ui.components.subcomponents.permissionsoverview.683fbe2168")}</p>
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <div className="p-4 bg-sand-50/70 border border-sand-200/80 rounded-xl flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between text-xs text-bark-600">
              <div className="flex flex-wrap items-center gap-4 text-left">
                <span className="font-bold font-mono text-[10px] uppercase text-bark-500">{pfUiT("ui.components.subcomponents.permissionsoverview.6bbc0a01a0")}</span>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {currentAllowedKeys.length === 0 && (
                          <span className="text-[11px] text-bark-400 italic">{pfUiT("ui.components.subcomponents.permissionsoverview.5811831495")}</span>
                        )}
                      </div>
                    </div>
        ...
        {activeTab === 'json' && (
          ...
          >
            ...
            <div className="p-4 bg-indigo-50/40 border border-indigo-150/50 rounded-xl flex items-start gap-3">
              ...
              <div>
                <strong className="text-xs text-indigo-950 font-serif font-bold block">{pfUiT("ui.components.subcomponents.permissionsoverview.1811fe8f1b")}</strong>
                <p className="text-[11px] text-bark-650 leading-relaxed font-sans">{pfUiT("ui.components.subcomponents.permissionsoverview.c0cf8c9e66")}</p>
              </div>
            </div>
            ...
            <div className="relative border border-stone-800 rounded-xl overflow-hidden bg-stone-950 shadow-lg font-mono text-xs">
              ...
              <div className="bg-stone-900 border-b border-stone-850 px-4 py-2 flex items-center justify-between text-stone-400 text-[10px] font-mono uppercase tracking-wider font-semibold">
                <div className="flex items-center gap-2">
                  ...
                  <span className="w-2.5 h-2.5 bg-emerald-500 rounded-full inline-block"></span>
                  <span className="ml-2 font-bold text-stone-300">{pfUiT("ui.components.subcomponents.permissionsoverview.20c1b24c2e")}</span>
                </div>
                <div className="flex items-center gap-2">
      ...
      <div className="pt-4 border-t border-sand-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-[10px] font-mono text-bark-450 text-left">
        <div className="flex items-center gap-1.5">
          ...
          <span>{pfUiT("ui.components.subcomponents.permissionsoverview.d89bbcd04f")}<strong className="font-bold text-bark-600">{meta.engine}</strong></span>
        </div>
        <div>
          <span>{pfUiT("ui.components.subcomponents.permissionsoverview.1067a8d715")}<strong className="font-bold text-bark-600">{meta.lastUpdated}</strong></span>
        </div>
      </div>
