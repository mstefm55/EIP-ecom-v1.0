...
export default function CreationsAndFeedback({
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      const newPostId = createClientRecordId('community-post');
      ...
      if (targetIdToUse !== 'atelier' && onAddReview) {
        const reviewObject = {
          id: createClientRecordId('product-review'),
          name: formName.trim(),
          rating: formRating,
  ...
  const handleAddReply = (postId) => {
    ...
    const newReply = {
      id: createClientRecordId('community-reply'),
      author: authorName,
      comment: text.trim(),
  ...
  const patternGalleryMakes = useMemo(() => {
    ...
    let list = posts.filter(p => p.type === 'creation' && p.targetId === currentPatternId);
    ...
    return list;
  }, [posts, isPatternMode, currentPatternId, sortBy]);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-sand-200/60 pb-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[9px] text-clay-700 font-bold font-mono uppercase tracking-[0.18em] bg-clay-50 border border-clay-100 px-2 py-0.5 rounded">{pfUiT("ui.components.creationsandfeedback.1d028977b5")}</span>
            ...
            <p className="text-[11px] text-bark-500 leading-normal font-sans">{pfUiT("ui.components.creationsandfeedback.c6808ad127")}<span className="font-bold text-bark-850">{pattern.name}</span>.
            </p>
          </div>

          {/* Local Tab Switcher */}
          <div className="grid grid-cols-2 bg-sand-100 p-0.5 rounded-[4px] border border-sand-200/60 w-full max-w-full min-w-0" id="pattern-tab-nav">
            <button
              onClick={() => { setActiveTab('gallery'); setShowPatternReviewForm(false); }}
              className={`min-w-0 px-2 py-1.5 text-[11px] font-bold font-mono uppercase tracking-[0.04em] rounded-[3px] transition-all cursor-pointer flex items-center justify-center gap-1 ${
                activeTab === 'gallery' && !showPatternReviewForm ? 'bg-white text-clay-700 shadow-3xs' : 'text-bark-500 hover:text-bark-800'
              }`}
            >
              <ImageIcon className="w-3.5 h-3.5 shrink-0" />
              <span className="min-w-0 text-center leading-tight">Photos ({patternGalleryMakes.length})</span>
            </button>
            <button
            ...
            >
              ...
              <span className="min-w-0 text-center leading-tight">Written Reviews ({stats.count})</span>
            </button>
          </div>
        </div>
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 bg-sand-50/55 p-3.5 border border-sand-200 rounded-[4px]">
                  <div className="space-y-1">
                    <span className="text-xs font-bold text-bark-850 font-serif flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-[#ba6446]" />{pfUiT("ui.components.creationsandfeedback.6552573870")}</span>
                  </div>
                  ...
                  >
                    <Camera className="w-3.5 h-3.5" />{pfUiT("ui.components.creationsandfeedback.f9229f4e32")}</button>
                </div>
                ...
                {patternGalleryMakes.length === 0 ? (
                  <div className="text-center py-12 border border-dashed border-sand-200 rounded bg-sand-50/20" id="gallery-pattern-fallback">
                    ...
                    <p className="text-xs text-bark-450 italic font-medium">{pfUiT("ui.components.creationsandfeedback.6247ba091c")}</p>
                    <p className="text-[10px] text-bark-400 mt-0.5">{pfUiT("ui.components.creationsandfeedback.8994d78b8c")}</p>
                  </div>
                ) : (
                  <motion.div
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 bg-[#FCFAF7] border border-sand-200 rounded-[4px] p-4" id="pattern-rating-analytics">
                  ...
                  <div className="md:col-span-4 flex flex-col items-center justify-center text-center space-y-1 md:border-r md:border-sand-200/80 pr-2">
                    <span className="text-[8px] text-bark-450 uppercase tracking-widest font-mono font-bold">{pfUiT("ui.components.creationsandfeedback.9bccbe9d62")}</span>
                  ...
                  <div className="md:col-span-3 flex flex-col items-center justify-center space-y-2 text-center pl-2">
                    <span className="text-[10px] text-bark-500 font-sans leading-normal">{pfUiT("ui.components.creationsandfeedback.7b2139e61a")}</span>
                    <button
                      onClick={() => { setShowPatternReviewForm(true); setFormRating(5); }}
                    ...
                    >{pfUiT("ui.components.creationsandfeedback.35d20ca22d")}</button>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-sand-150 pb-3 text-xs font-sans text-bark-700">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[10px] font-mono font-bold text-bark-400 uppercase tracking-widest flex items-center gap-1">
                      <Filter className="w-3 h-3 text-clay-605" />{pfUiT("ui.components.creationsandfeedback.2d64c616fc")}</span>

                    <div className="flex gap-1" id="filter-rating-buttons">
                      {[
                        { id: 'all', label: pfUiT('ui.creations.filters.reviews.all', {}, 'All Reviews') },
                        { id: '5', label: pfUiT('ui.creations.filters.reviews.fiveStars', {}, '5 Stars') },
                        { id: '4', label: pfUiT('ui.creations.filters.reviews.fourStars', {}, '4 Stars') },
                        { id: 'photo', label: pfUiT('ui.creations.filters.reviews.withPhotos', {}, 'With Photos') }
                      ].map((opt) => (
                        <button
                          key={opt.id}
                  ...
                  <div className="flex items-center gap-1.5 text-[10.5px]">
                    <span className="text-bark-400">{pfUiT("ui.components.creationsandfeedback.cf906ac0cc")}</span>
                    <select
                      value={sortBy}
                    ...
                    >
                      <option value="newest">{pfUiT("ui.components.creationsandfeedback.2136f56251")}</option>
                      <option value="highest">{pfUiT("ui.components.creationsandfeedback.5eaf347f94")}</option>
                      <option value="helpful">{pfUiT("ui.components.creationsandfeedback.1729719ba3")}</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="space-y-4 max-h-[380px] overflow-y-auto pr-1" id="pattern-reviews-feed">
                  {patternReviews.length === 0 ? (
                    <div className="text-center py-10 border border-dashed border-sand-200 rounded-[4px]">
                      <p className="text-xs text-bark-450 italic font-sans">{pfUiT("ui.components.creationsandfeedback.84ad39d52e")}</p>
                    </div>
                  ) : (
                      ...
                      <div key={rev.id} className="bg-sand-50/20 border border-sand-150 p-4 rounded-[4px] space-y-3 transition-all hover:bg-sand-50/40" id={`pattern-review-card-${rev.id}`}>
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                          <div>
                            ...
                            <p className="text-[10px] text-bark-400 font-sans mt-0.5">{pfUiT("ui.components.creationsandfeedback.feb3a3b760")}<span className="font-bold text-bark-700">{rev.name}</span> • <span>{rev.date}</span>
                            </p>
                          </div>
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pt-1">
                          {rev.fabric && (
                            <div className="text-[9.5px] text-bark-500 font-sans flex items-center gap-1">
                              <span className="font-semibold text-bark-450 font-mono uppercase tracking-wider text-[8px]">{pfUiT("ui.components.creationsandfeedback.7f1545b433")}</span>
                          ...
                          {rev.image && (
                            ...
                            >
                              <ImageIcon className="w-3 h-3" />{pfUiT("ui.components.creationsandfeedback.b64b11826f")}</div>
                          )}
                        </div>
                        ...
                        <div className="border-t border-sand-150 pt-2 flex justify-between items-center text-[10px] text-bark-400 font-sans">
                          ...
                          <span className="text-[9px] font-mono text-bark-400">{pfUiT("ui.components.creationsandfeedback.891cf66866")}</span>
                        </div>
                      </div>
                    ))
                  )}
              ...
              >
                <div className="flex justify-between items-center border-b border-sand-150 pb-2.5">
                  <div className="space-y-0.5">
                    <h4 className="font-serif text-sm font-semibold text-bark-900 flex items-center gap-1.5">
                      <Sparkle className="w-4 h-4 text-clay-650 animate-pulse" />{pfUiT("ui.components.creationsandfeedback.3972132a6b")}</h4>
                    <p className="text-[9.5px] text-bark-450 font-sans">{pfUiT("ui.components.creationsandfeedback.323b0f3035")}</p>
                  </div>
                  <button
                    type="button"
                ...
                {submitSuccess ? (
                  <div className="p-5 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-[4px] flex items-center gap-3.5 text-xs font-sans" id="make-success-alert">
                    ...
                    <div>
                      <p className="font-bold">{pfUiT("ui.components.creationsandfeedback.7f59ba51fb")}</p>
                  ...
                  <div className="space-y-4">
                      ...
                      >
                        ...
                        {uploadedImage ? (
                          <div className="relative w-full max-w-[260px] aspect-[4/3] rounded overflow-hidden shadow-2xs border border-sand-200">
                            <img
                              src={uploadedImage}
                              alt={pfUiT("ui.components.creationsandfeedback.fd5d8f8df0")}
                            ...
                            <button
                              ...
                              title={pfUiT("ui.components.creationsandfeedback.0e4bf5fc60")}
                            >
                          ...
                          <div className="py-2 flex flex-col items-center space-y-1.5">
                            ...
                            <div className="text-xs text-bark-800">
                              <span className="font-bold text-clay-700">{pfUiT("ui.components.creationsandfeedback.ce8363ae6f")}</span> or drag &amp; drop photo here
                            </div>
                            <span className="text-[9px] text-bark-400 font-sans leading-none">{pfUiT("ui.components.creationsandfeedback.2863f5d5bc")}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div>
                        <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block mb-1" htmlFor="maker-name-input">{pfUiT("ui.components.creationsandfeedback.79c2731b02")}</label>
                        <input
                          id="maker-name-input"
                          type="text"
                          required
                          placeholder={pfUiT("ui.components.creationsandfeedback.535d436bd2")}
                          value={formName}
                      ...
                      <div>
                        <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block mb-1" htmlFor="maker-size-select">{pfUiT("ui.components.creationsandfeedback.438504901e")}</label>
                        <select
                      ...
                      <div>
                        <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block mb-1" htmlFor="maker-difficulty-select">{pfUiT("ui.components.creationsandfeedback.191413bc67")}</label>
                        <select
                          ...
                          className="w-full bg-white border border-sand-200 rounded-[4px] px-2 py-1.5 text-xs text-bark-800 focus:outline-none focus:border-clay-500 cursor-pointer"
                        >
                          <option value="Easy">{pfUiT("ui.components.creationsandfeedback.34bc6ddf87")}</option>
                          <option value="Just Right">{pfUiT("ui.components.creationsandfeedback.e03383344b")}</option>
                          <option value="Challenging">{pfUiT("ui.components.creationsandfeedback.bdd8c5e422")}</option>
                        </select>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block mb-1" htmlFor="maker-fabric-input">{pfUiT("ui.components.creationsandfeedback.2845658007")}</label>
                        <input
                          id="maker-fabric-input"
                          type="text"
                          placeholder={pfUiT("ui.components.creationsandfeedback.4db557e3d6")}
                          value={formFabric}
                      ...
                      <div>
                        <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block mb-1">{pfUiT("ui.components.creationsandfeedback.cbb7158a7b")}</label>
                    ...
                    <div>
                      <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block mb-1" htmlFor="maker-title-input">{pfUiT("ui.components.creationsandfeedback.0b231f9af8")}</label>
                      <input
                        id="maker-title-input"
                        type="text"
                        required
                        placeholder={pfUiT("ui.components.creationsandfeedback.980c00caf9")}
                        value={formTitle}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        required
                        rows={3}
                        placeholder={pfUiT("ui.components.creationsandfeedback.946c9a38d3")}
                        value={formComment}
                    ...
                    <div className="flex gap-2.5 pt-2">
                      ...
                      >{pfUiT("ui.components.creationsandfeedback.f05ec1773e")}</button>
                      <button
                        type="submit"
                      ...
                      >
                        {isSubmitting ? (
                          ...
                          <span className="flex items-center gap-1.5">
                            <Check className="w-3.5 h-3.5" />{pfUiT("ui.components.creationsandfeedback.e399f2f1d9")}</span>
                        )}
                      </button>
                    </div>
    ...
    <section className="bg-white rounded-[4px] border border-sand-200 p-6 md:p-10 space-y-8 shadow-lux relative overflow-hidden" id="atelier-showroom-feedback-section">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-sand-150/85">
        <div className="space-y-1 text-left">
          ...
          <p className="text-xs text-bark-550 max-w-xl leading-relaxed mt-1 font-sans">{pfUiT("ui.components.creationsandfeedback.3836718ee6")}</p>
        </div>
          ...
          >
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-4" id="spotlight-header">
                  <div className="space-y-1 text-left">
                    <div className="flex items-center gap-2 text-clay-700 font-mono text-[10px] uppercase tracking-widest font-bold">
                      ...
                      <span>{pfUiT("ui.components.creationsandfeedback.a2ba273657")}</span>
                    </div>
                    <h4 className="font-serif text-xl md:text-2xl text-bark-900 tracking-tight">{pfUiT("ui.components.creationsandfeedback.e9ac653b33")}</h4>
                    <p className="text-xs text-bark-500 max-w-xl">{pfUiT("ui.components.creationsandfeedback.bf8b713852")}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0 self-start md:self-auto" id="spotlight-arrows">
                    <button
                      ...
                      aria-label={pfUiT("ui.components.creationsandfeedback.50f7b4e376")}
                    ...
                    <button
                      ...
                      className="p-1.5 border border-sand-200 rounded-[4px] bg-white text-bark-700 hover:bg-sand-100 hover:text-bark-900 transition-colors shadow-2xs cursor-pointer"
                      aria-label={pfUiT("ui.components.creationsandfeedback.1aa4aeb386")}
                        ...
                        <div className="space-y-1.5 text-white">
                          ...
                          <div className="flex justify-between items-center pt-2 border-t border-white/10 mt-2 text-[9px] text-sand-200 font-sans">
                            <div className="flex gap-3">
                              <span>{pfUiT("ui.components.creationsandfeedback.83cbf187e7")}<strong className="text-white">{spotlightCreations[carouselIndex].fabric || 'Couture fabric'}</strong></span>
                              {spotlightCreations[carouselIndex].size && (
                                <span>{pfUiT("ui.components.creationsandfeedback.0de9d20cf4")}<strong className="text-white">{spotlightCreations[carouselIndex].size}</strong></span>
                              )}
                            </div>
                            <button
                              type="button"
                      ...
                      <blockquote className="space-y-2">
                        ...
                        {spotlightCreations[carouselIndex].tips && spotlightCreations[carouselIndex].tips !== spotlightCreations[carouselIndex].comment && (
                          <div className="mt-3 bg-sand-100/50 p-3 rounded border border-sand-200/50 text-xs text-bark-750 font-sans">
                            <strong className="text-[10px] font-mono text-clay-705 uppercase tracking-wider block mb-1">{pfUiT("ui.components.creationsandfeedback.5fdf5006a9")}</strong>
                      ...
                      <div className="flex items-center gap-3 pt-3 border-t border-sand-200/80" id="spotlight-author-row">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 text-bark-500 text-[10px] font-sans mt-0.5">
                            <span>{pfUiT("ui.components.creationsandfeedback.afc05f0667")}</span>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3.5 bg-[#FAF8F5] border border-sand-200/80 p-3 rounded-lg" id="showroom-controls">
              ...
              <div className="flex items-center gap-1.5 shrink-0 overflow-x-auto">
                <span className="text-[10px] text-bark-450 font-bold font-mono uppercase tracking-wider hidden lg:inline">{pfUiT("ui.components.creationsandfeedback.06c7495413")}</span>
                {[
                  { id: 'all', label: pfUiT('ui.creations.filters.entries.all', {}, 'All Entries') },
                  { id: 'creations', label: pfUiT('ui.creations.filters.entries.creations', {}, 'Sewn Makes') },
                  { id: 'atelier-feedback', label: pfUiT('ui.creations.filters.entries.feedback', {}, 'Feedback') }
                ].map((opt) => (
                  <button
                    key={opt.id}
            ...
            {/* Posts feed grid */}
            <div className="space-y-5 text-left" id="showroom-posts-grid">
              {filteredGlobalPosts.length === 0 ? (
                <div className="lg:col-span-2 text-center py-16 border border-dashed border-sand-200 rounded-lg bg-sand-50/25">
                  ...
                  <h4 className="font-serif text-base font-semibold text-bark-900">{pfUiT("ui.components.creationsandfeedback.11a6e1eddb")}</h4>
                  <p className="text-xs text-bark-450 max-w-sm mx-auto leading-normal font-sans mt-1">{pfUiT("ui.components.creationsandfeedback.beda14089a")}</p>
                </div>
              ) : (
                  ...
                  >
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 text-[9.5px]">
                            {post.fabric && (
                              <span className="bg-[#FAF8F5] border border-sand-200 px-2.5 py-0.5 rounded text-bark-700 font-sans">
                                <strong className="text-[8px] font-mono text-bark-450 uppercase mr-1">{pfUiT("ui.components.creationsandfeedback.83cbf187e7")}</strong>
                                {post.fabric}
                              </span>
                            )}
                            {post.size && (
                    ...
                    <div className="border-t border-sand-150 pt-3 space-y-3">
                      <div className="flex items-center justify-between text-[11px] text-bark-500 font-sans">
                        ...
                        <span className="text-[9.5px] text-bark-400 font-mono">{pfUiT("ui.components.creationsandfeedback.353869e47b")}</span>
                      </div>
                            ...
                            return (
                              <div key={reply.id} className="text-[10.5px] leading-normal font-sans border-l-2 border-sand-200 pl-2.5 py-0.5 space-y-0.5">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  {isStaff && (
                                    <span className="text-[7px] bg-clay-50 border border-clay-150/40 text-clay-705 font-bold font-mono px-1 rounded-full">{pfUiT("ui.components.creationsandfeedback.313fc75cc8")}</span>
                                  )}
                      ...
                      <AnimatePresence>
                        {activeReplyBox === post.id && (
                          ...
                          >
                            <input
                              type="text"
                              placeholder={pfUiT("ui.components.creationsandfeedback.a105ff0918")}
            ...
            {submitSuccess ? (
              <div className="bg-sand-50/30 border border-sand-200 rounded-lg p-8 text-center space-y-6" id="feedback-success-card">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-bark-550 font-sans max-w-sm mx-auto">{pfUiT("ui.components.creationsandfeedback.0771f41151")}</p>
                </div>
                ...
                {atelierReply && (
                  <div className="bg-white border border-sand-150 p-4 rounded-md text-left relative overflow-hidden shadow-2xs max-w-lg mx-auto">
                    ...
                    <span className="text-[8px] font-bold font-mono tracking-wider text-clay-705 uppercase bg-clay-50 px-2 py-0.5 rounded inline-block mb-2">{pfUiT("ui.components.creationsandfeedback.00cf0c1725")}</span>
                ...
                <div className="flex justify-center gap-3 pt-3">
                  ...
                  >{pfUiT("ui.components.creationsandfeedback.df638fa153")}</button>
                  <button
                  ...
                  >{pfUiT("ui.components.creationsandfeedback.4edb497ddf")}</button>
                </div>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="bg-sand-50/20 border border-sand-250 rounded-lg p-6 md:p-8 space-y-6" id="central-creation-feedback-form">
                ...
                {/* Mode toggle */}
                <div className="space-y-1.5">
                  <label className="text-[10px] text-bark-450 font-mono font-bold uppercase tracking-wider block">{pfUiT("ui.components.creationsandfeedback.fcdea15db4")}</label>
                  <div className="grid grid-cols-2 gap-3" id="selection-target-btns">
                    ...
                    >
                      <strong className="text-xs font-serif font-bold">{pfUiT("ui.components.creationsandfeedback.2572ab78e2")}</strong>
                      <span className="text-[9.5px] leading-tight font-sans text-bark-500 mt-1">{pfUiT("ui.components.creationsandfeedback.e850f32878")}</span>
                    </button>
                    ...
                    >
                      <strong className="text-xs font-serif font-bold">{pfUiT("ui.components.creationsandfeedback.5644052f75")}</strong>
                      <span className="text-[9.5px] leading-tight font-sans text-bark-500 mt-1">{pfUiT("ui.components.creationsandfeedback.2ccac6170d")}</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {creationTarget !== 'atelier' ? (
                    <div className="space-y-1">
                      <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block mb-1" htmlFor="creation-pattern-select">{pfUiT("ui.components.creationsandfeedback.ff1687a824")}</label>
                      <select
                    ...
                    <div className="space-y-1">
                      <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block mb-1" htmlFor="feedback-topic-select">{pfUiT("ui.components.creationsandfeedback.f829fcaafb")}</label>
                      <select
                      ...
                      >
                        ...
                        <option value="instruction-clarity">{pfUiT("ui.components.creationsandfeedback.f4ca0998a4")}</option>
                        <option value="pattern-request">{pfUiT("ui.components.creationsandfeedback.5d7962288d")}</option>
                        <option value="general">{pfUiT("ui.components.creationsandfeedback.15d0aaec5b")}</option>
                      </select>
                    </div>
                  )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block" htmlFor="designer-name-input">{pfUiT("ui.components.creationsandfeedback.954702c736")}</label>
                    <input
                      id="designer-name-input"
                      type="text"
                      required
                      placeholder={pfUiT("ui.components.creationsandfeedback.ee01305714")}
                      value={formName}
                      onChange={(e) => setFormName(e.target.value)}
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      type="email"
                      placeholder={pfUiT("ui.components.creationsandfeedback.69cbf95cda")}
                      value={formEmail}
                ...
                {creationTarget !== 'atelier' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block" htmlFor="form-fabric-input">{pfUiT("ui.components.creationsandfeedback.b65274c401")}</label>
                      <input
                        id="form-fabric-input"
                        type="text"
                        placeholder={pfUiT("ui.components.creationsandfeedback.fb9d26010f")}
                        value={formFabric}
                        onChange={(e) => setFormFabric(e.target.value)}
                        className="w-full bg-white border border-sand-200 rounded-lg px-3 py-2 text-xs text-bark-800 focus:outline-none focus:border-clay-500 font-sans shadow-3xs"
                      />
                    ...
                    <div className="space-y-1">
                      <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block" htmlFor="form-size-select">{pfUiT("ui.components.creationsandfeedback.69ce41f9e8")}</label>
                      <select
                        id="form-size-select"
                        value={formSize}
                        onChange={(e) => setFormSize(e.target.value)}
                    ...
                    >
                      ...
                      {uploadedImage ? (
                        <div className="relative w-full max-w-[280px] aspect-[16/9] rounded overflow-hidden shadow-xs border border-sand-200">
                          <img
                            src={uploadedImage}
                            alt={pfUiT("ui.components.creationsandfeedback.61beeb0684")}
                            className="w-full h-full object-cover"
                            referrerPolicy="no-referrer"
                          />
                          <button
                        ...
                        <div className="py-1 flex flex-col items-center space-y-2">
                          ...
                          <div className="text-xs text-bark-800 font-sans">
                            <span className="font-bold text-clay-700">{pfUiT("ui.components.creationsandfeedback.a2611203e4")}</span>{pfUiT("ui.components.creationsandfeedback.a68f96e466")}</div>
                          <span className="text-[9px] text-bark-400 leading-none">{pfUiT("ui.components.creationsandfeedback.7a187b0268")}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block" htmlFor="form-title-input">{pfUiT("ui.components.creationsandfeedback.ac04c1d62b")}</label>
                    <input
                      id="form-title-input"
                      type="text"
                      required
                      placeholder={pfUiT("ui.components.creationsandfeedback.9f75b826e3")}
                      value={formTitle}
                      onChange={(e) => setFormTitle(e.target.value)}
                      className="w-full bg-white border border-sand-200 rounded-lg px-3 py-2 text-xs text-bark-800 focus:outline-none focus:border-clay-500 font-sans shadow-3xs"
                    />
                  ...
                  <div className="space-y-1">
                    <label className="text-[9.5px] text-bark-450 font-mono font-bold uppercase tracking-wider block" htmlFor="form-comment-textarea">{pfUiT("ui.components.creationsandfeedback.c166d7d53d")}</label>
                    <textarea
                  ...
                  >
                    {isSubmitting ? (
                      <>
                        ...
                        <span>{pfUiT("ui.components.creationsandfeedback.3f652b5822")}</span>
                      </>
                    ) : (
                      <>
                        <Check className="w-4 h-4" />
                        <span>{pfUiT("ui.components.creationsandfeedback.4d4a8c9f27")}</span>
                      </>
                    )}
                  </button>
                </div>
      ...
      <AnimatePresence id="lightbox-overlay-block">
        {activeLightbox && (
          <motion.div
            ...
            className="fixed inset-0 bg-bark-950/90 backdrop-blur-md flex items-center justify-center p-4 cursor-zoom-out"
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-[4px] overflow-hidden w-[96vw] max-w-[1380px] h-[92vh] grid grid-cols-1 md:grid-cols-12 shadow-2xl relative cursor-default text-left"
...
id="lightbox-card"
            >
              ...
              <button
                ...
                title={pfUiT("ui.components.creationsandfeedback.ff893fc455")}
              >
                <X className="w-4 h-4" />
              </button>

              {/* Large image left column */}
              <div className="md:col-span-7 bg-black flex items-center justify-center h-[42vh] md:h-full overflow-hidden relative">
                <img
              ...
              <div className="md:col-span-5 p-5 md:p-7 flex flex-col justify-between overflow-y-auto h-full space-y-5" id="lightbox-details">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 border-t border-sand-200/80 pt-3 text-xs leading-relaxed text-bark-750">
                    <div>
                      <span className="font-bold font-mono text-[9px] text-bark-400 uppercase tracking-widest block mb-0.5">{pfUiT("ui.components.creationsandfeedback.7d7776357d")}</span>
                  ...
                  {activeLightbox.fabric && (
                    <div className="text-xs font-sans">
                      <span className="font-bold font-mono text-[9px] text-bark-400 uppercase tracking-widest block mb-1">{pfUiT("ui.components.creationsandfeedback.2845658007")}</span>
                ...
                <div className="border-t border-sand-200/80 pt-4 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-bark-400 font-mono">{pfUiT("ui.components.creationsandfeedback.581f0c5ab0")}</span>
                </div>
