...
const getDifficultyDetails = (difficulty) => {
  const copyMap = localizeMetadataTree(
    perfectFitMetadata.componentUi.patternDifficulty.gallery,
    'component.patternDifficulty.gallery',
    pfUiT
  );
  const copy = copyMap[difficulty] || { ...copyMap.default, label: difficulty || copyMap.default?.label || 'Intermediate' };
  if (difficulty === 'Beginner') return { ...copy, icon: Compass, badgeClasses: 'bg-emerald-50 text-emerald-800 border-emerald-200/60', dotClass: 'bg-emerald-500', accentColor: '#10b981' };
  if (difficulty === 'Advanced') return { ...copy, icon: Sparkles, badgeClasses: 'bg-rose-50/90 text-rose-800 border-rose-200/60', dotClass: 'bg-rose-500', accentColor: '#f43f5e' };
  if (difficulty === 'Intermediate') return { ...copy, icon: Scissors, badgeClasses: 'bg-amber-50 text-amber-800 border-amber-200/60', dotClass: 'bg-amber-500', accentColor: '#f59e0b' };
  return { ...copy, icon: Scissors, badgeClasses: 'bg-sand-50 text-bark-800 border-sand-200/60', dotClass: 'bg-bark-500', accentColor: '#ba6446' };
};
...
export default function PatternImageGallery({
  ...
  const [hoverEnabled, setHoverEnabled] = useState(() => {
    const saved = clientPreferences.getItem('atelier_hover_info_enabled');
  ...
  return (
    <div className="space-y-6" id="pattern-showcase-gallery-component">
      ...
      <div>
        <p className="text-xs text-bark-500 italic max-w-lg leading-relaxed font-sans">{pfUiT("ui.components.patternimagegallery.6ce010a2de")}</p>
      </div>
      ...
      <div className="bg-[#FAF8F5] border border-sand-200/70 p-5 rounded-xl space-y-4" id="gallery-premium-filters-panel">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-sand-200/50">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-mono font-bold uppercase tracking-wider text-bark-800">{pfUiT("ui.components.patternimagegallery.46c805308d")}</h4>
          </div>
          <div className="flex items-center gap-2">
            ...
            {(selectedDifficulty !== 'All' || selectedCategory !== 'All' || selectedPriceRange !== 'All' || searchQuery !== '') && (
              ...
              >{pfUiT("ui.components.patternimagegallery.cb074130eb")}</button>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-1.5">
            <label className="text-[9.5px] font-mono font-bold uppercase tracking-wider text-bark-500 block">{pfUiT("ui.components.patternimagegallery.2ea9d2f46f")}</label>
          ...
          <div className="space-y-1.5">
            <label className="text-[9.5px] font-mono font-bold uppercase tracking-wider text-bark-500 block">{pfUiT("ui.components.patternimagegallery.71e9aa9a64")}</label>
            <div className="flex flex-wrap bg-sand-100/65 p-1 rounded-lg border border-sand-200/60 gap-0.5">
          ...
          {/* Price Range Filter */}
          <div className="space-y-1.5">
            <label className="text-[9.5px] font-mono font-bold uppercase tracking-wider text-bark-500 block">{pfUiT("ui.components.patternimagegallery.560b9cddfb")}</label>
            <div className="flex flex-wrap bg-sand-100/65 p-1 rounded-lg border border-sand-200/60 gap-0.5">
              {[
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-sand-50/50 p-4 border border-sand-200/60 rounded-lg" id="showcase-filters">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder={pfUiT("ui.components.patternimagegallery.0bd69b0e0f")}
            value={searchQuery}
      ...
      {filteredPatterns.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-lg border border-dashed border-sand-300 flex flex-col items-center justify-center space-y-2">
          ...
          <p className="text-xs font-semibold text-bark-900 font-serif">{pfUiT("ui.components.patternimagegallery.5eb0383d40")}</p>
          <button
            ...
            type="button"
          >{pfUiT("ui.components.patternimagegallery.e7c4c5f35b")}</button>
        </div>
      ) : (
        <>
  ...
  catalogColumns === 3 ? 'xl:grid-cols-3' : 'xl:grid-cols-4'
              ...
              >
                    ...
                    <div className={isHovered ? 'opacity-0 pointer-events-none' : 'opacity-100 transition-opacity duration-300'}>
                      <Tooltip content="Instantly overlay technical drawings, size keys, and active user reviews." position="bottom">
                        <button
                          ...
                          title={pfUiT("ui.components.patternimagegallery.ba93586faa")}
                          type="button"
                        >
                          <Eye className="w-3.5 h-3.5 shrink-0" />
                          <span>{pfUiT("ui.components.patternimagegallery.cdd30c3e04")}</span>
                        </button>
                      </Tooltip>
                    </div>
                  </div>
                ...
                <div className={`absolute bottom-4 inset-x-4 bg-white/95 backdrop-blur-md border border-sand-200/80 p-3 rounded-[3px] shadow-md transition-all duration-300 z-10 flex flex-col ${
                  ...
                  <div className="flex items-center justify-between mt-1 text-[10px] text-bark-550 font-sans border-t border-sand-100 pt-1.5">
                    <span className="font-mono text-[8px] uppercase tracking-wider text-bark-400">{pfUiT("ui.components.patternimagegallery.56b6278d58")}</span>
                    ...
                    >
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between border-b border-white/10 pb-2">
                          <div className="leading-none">
                            ...
                            <span className="text-[5.5px] font-mono tracking-[0.2em] text-sand-400 font-bold uppercase mt-0.5 block">{pfUiT("ui.components.patternimagegallery.212aad4068")}</span>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-[10px] font-mono border-t border-white/10 pt-2 text-sand-400">
                          <Tooltip content="Tailored recommendation for fabric fibers matching this pattern's draping parameters." position="top">
                            <div className="space-y-0.5 cursor-help">
                              <span className="block text-[8px] uppercase tracking-wider font-semibold text-white/40">{pfUiT("ui.components.patternimagegallery.79aa0c29a7")}</span>
                          ...
                          <Tooltip content="Comprehensive sizing chart. Size adjustments and layers are built directly into standard digital sheets." position="top">
                            <div className="space-y-0.5 cursor-help">
                              <span className="block text-[8px] uppercase tracking-wider font-semibold text-white/40">{pfUiT("ui.components.patternimagegallery.5d67286d96")}</span>
                              <span className="block text-sand-200 font-sans text-[9.5px]">{pfUiT("ui.components.patternimagegallery.63ab6dd4a6")}</span>
                            </div>
                          </Tooltip>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2.5 pt-3 border-t border-white/10">
                        ...
                        <div className="flex items-center justify-between text-xs font-mono text-sand-300">
                          <span>{pfUiT("ui.components.patternimagegallery.b7e48e8744")}</span>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <Tooltip content="Explore high-fidelity sizing tables, interactive swatch customizers, and print guides." position="top">
                            <button
                              ...
                              type="button"
                              title={pfUiT("ui.components.patternimagegallery.e14135e83c")}
                            >
                              <Compass className="w-3 h-3" />
                              <span>{pfUiT("ui.components.patternimagegallery.abce5718ee")}</span>
                            </button>
                          </Tooltip>
                          ...
                          <Tooltip content="Instantly purchase and download the standard multi-size high-resolution PDF." position="top">
                            <button
                              ...
                              type="button"
                              title={pfUiT("ui.components.patternimagegallery.f4a443ccd9")}
                            >
                              {addingToCartId === pattern.id ? (
                                ...
                                <>
                                  ...
                                  <span>{pfUiT("ui.components.patternimagegallery.b15a57b6b5")}</span>
                                </>
                              )}
                            </button>
                          </Tooltip>
                        ...
                        <button
                          ...
                          type="button"
                        >
                          <span>{pfUiT("ui.components.patternimagegallery.47f8bf9c8d")}</span>
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between pt-6 border-t border-sand-200/80 gap-4" id="showcase-pagination-controls">
            <span className="text-[11px] font-mono text-bark-500">{pfUiT("ui.components.patternimagegallery.7e6bdd8c97")}<span className="font-bold text-bark-800">{(currentPage - 1) * itemsPerPage + 1}</span>–<span className="font-bold text-bark-800">{Math.min(filteredPatterns.length, currentPage * itemsPerPage)}</span>{pfUiT("ui.components.patternimagegallery.572ed9a31e")}<span className="font-bold text-bark-800">{filteredPatterns.length}</span>{pfUiT("ui.components.patternimagegallery.ffd4969ae1")}</span>
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                title={pfUiT("ui.components.patternimagegallery.9caa29b6ed")}
                type="button"
              >
              ...
              <button
                ...
                disabled={currentPage === 1}
                className="p-1.5 rounded border border-sand-250 bg-white hover:bg-[#FAF8F5] disabled:opacity-40 transition-all cursor-pointer text-bark-600 disabled:cursor-not-allowed"
                title={pfUiT("ui.components.patternimagegallery.a8a50aa926")}
                type="button"
              >
              ...
              <button
                ...
                className="p-1.5 rounded border border-sand-250 bg-white hover:bg-[#FAF8F5] disabled:opacity-40 transition-all cursor-pointer text-bark-600 disabled:cursor-not-allowed"
                title={pfUiT("ui.components.patternimagegallery.c6faa80fa2")}
                type="button"
              >
              ...
              <button
                ...
                disabled={currentPage === totalPages}
                className="p-1.5 rounded border border-sand-250 bg-white hover:bg-[#FAF8F5] disabled:opacity-40 transition-all cursor-pointer text-bark-600 disabled:cursor-not-allowed"
                title={pfUiT("ui.components.patternimagegallery.89d8e5d2e1")}
                type="button"
              >
