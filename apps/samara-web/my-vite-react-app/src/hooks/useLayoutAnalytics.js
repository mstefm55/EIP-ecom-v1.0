...
export function useLayoutAnalytics() {
  ...
  const fetchLogs = useCallback(() => {
    try {
      const saved = runtimeDataStorage.getItem(LOCAL_STORAGE_KEY);
  ...
  const trackInteraction = useCallback((actionType, details = {}) => {
    try {
      ...
      runtimeDataStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(updatedLogs));
      setLogs(updatedLogs);
  ...
  const clearAnalytics = useCallback(() => {
    try {
      runtimeDataStorage.removeItem(LOCAL_STORAGE_KEY);
      setLogs([]);
