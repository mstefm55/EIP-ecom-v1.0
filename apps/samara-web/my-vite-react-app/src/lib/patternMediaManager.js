...
export function getAllPatternSwatches() {
  try {
    const saved = runtimeDataStorage.getItem(SWATCH_STORAGE_KEY);
    if (saved) {
...
export function savePatternSwatches(patternId, swatchList) {
  ...
  try {
    runtimeDataStorage.setItem(SWATCH_STORAGE_KEY, JSON.stringify(allSwatches));
...
export function getAllPatternMedia() {
  try {
    const saved = runtimeDataStorage.getItem(STORAGE_KEY);
    if (saved) {
      const parsed = JSON.parse(saved);
      if (parsed && typeof parsed === 'object') {
...
export function savePatternMedia(patternId, mediaList) {
  ...
  try {
    runtimeDataStorage.setItem(STORAGE_KEY, JSON.stringify(allMedia));
