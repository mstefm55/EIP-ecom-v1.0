    ...
    <div className="space-y-6" id="workspace-analytics-dashboard">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-2xl border border-sand-200 shadow-ux">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono uppercase tracking-widest text-clay-700 font-bold">
              DynamicLayout Auditing Telemetry
            </span>
          </div>
          <h3 className="text-lg font-serif text-bark-950 font-light">
            Security & Interaction Analytics Ledger
          </h3>
          <p className="text-xs text-bark-550 max-w-lg">
            Tracks interface adjustments, credentials hopping, and unauthorized component rendering attempts to capture workflow dynamics.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
          <button
            ...
            title="Refresh logs from local cache"
          >
          ...
          <button
            ...
            title="Purge all logged interactions"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl border border-sand-200 shadow-4xs space-y-2">
          <span className="text-[9px] font-mono uppercase tracking-wider text-bark-400 block">Total Logs Captured</span>
          <div className="flex items-baseline gap-1.5">
            <strong className="text-2xl font-serif text-bark-900">{metrics.total}</strong>
            <span className="text-[10px] font-mono text-bark-500">Events</span>
          </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-sand-200 shadow-4xs space-y-2">
          <span className="text-[9px] font-mono uppercase tracking-wider text-bark-400 block">Security Denials</span>
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="text-[10px] font-mono text-bark-500">Gated Hits</span>
          </div>
          <div className="h-1 w-full bg-sand-100 rounded-full overflow-hidden">
            <div
        ...
        <div className="bg-white p-4 rounded-xl border border-sand-200 shadow-4xs space-y-2">
          <span className="text-[9px] font-mono uppercase tracking-wider text-bark-400 block">Most Active Persona</span>
        ...
        <div className="bg-white p-4 rounded-xl border border-sand-200 shadow-4xs space-y-2">
          <span className="text-[9px] font-mono uppercase tracking-wider text-bark-400 block">Gate Configurations</span>
          <div className="flex items-baseline gap-1.5">
            <strong className="text-2xl font-serif text-bark-900">{metrics.ruleToggledCount}</strong>
            <span className="text-[10px] font-mono text-bark-500">Toggles</span>
          </div>
          <div className="h-1 w-full bg-sand-100 rounded-full overflow-hidden">
            <div
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white border border-sand-200 rounded-xl p-5 space-y-4 shadow-ux">
            <div className="border-b border-sand-150 pb-2">
              <h4 className="text-xs font-serif font-bold text-bark-900 uppercase tracking-wider">Simulated Persona Usage</h4>
              <p className="text-[10px] text-bark-450 font-sans">Relative switch frequency metrics</p>
            </div>
          ...
          <div className="bg-white border border-sand-200 rounded-xl p-5 space-y-4 shadow-ux">
            <div className="border-b border-sand-150 pb-2">
              <h4 className="text-xs font-serif font-bold text-bark-900 uppercase tracking-wider">Workspace Action Breakdown</h4>
              <p className="text-[10px] text-bark-450 font-sans font-medium">Distribution across different tracked operations</p>
            </div>
            ...
            <div className="space-y-3.5 max-h-[180px] overflow-y-auto pr-1">
              ...
              {Object.keys(metrics.actionsMap).length === 0 && (
                <div className="text-center p-4 text-[10.5px] font-sans text-bark-400">
                  No actions registered yet. Try changing filters or switching simulated personas!
                </div>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-8 bg-white border border-sand-200 rounded-xl shadow-ux overflow-hidden flex flex-col">
          ...
          <div className="p-4 border-b border-sand-150 bg-sand-50/30 flex flex-col md:flex-row gap-3 justify-between items-center">
            ...
            <div className="relative w-full md:w-64">
              ...
              <input
                type="text"
                placeholder="Search raw payload or role..."
                value={searchTerm}
          ...
          <div className="divide-y divide-sand-100 max-h-[360px] overflow-y-auto flex-1">
                  ...
                  >
                    <div className="space-y-1 max-w-[75%]">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {/* Role Context */}
                        <span className="text-[9.5px] font-mono text-bark-600">
                          as <strong>{getRoleLabel(log.role)}</strong>
                        </span>
                      </div>
                      ...
                      <p className="text-[10.5px] text-bark-700 font-mono bg-sand-50/60 border border-sand-150 px-2 py-1 rounded">
                        {log.actionType === 'role_switched' && (
                          <span>Simulation sandbox persona changed to: <strong className="text-indigo-800">{log.details.nextRole}</strong></span>
                        )}
                        {log.actionType === 'rule_toggled' && (
                          <span>Gated component <strong className="text-amber-800">"{log.details.componentKey}"</strong> lock state updated for role <strong className="text-amber-800">"{log.details.roleId}"</strong></span>
                        )}
                        {log.actionType === 'access_denied' && (
                          <span>Rejected view block <strong className="text-rose-700">"{log.details.componentKey}"</strong>. Requires: <strong className="text-rose-700">[{log.details.requiredRoles?.join(', ')}]</strong></span>
                        )}
                        {log.actionType === 'feature_accessed' && (
                          <span>Rendered workspace block <strong className="text-emerald-800">"{log.details.componentName}"</strong> (<span className="font-mono text-[9.5px] text-emerald-700">{log.details.componentKey}</span>)</span>
                        )}
                        {log.actionType === 'config_toggled' && (
                          <span>Operations engine controller expanded state altered to: <strong className="text-stone-700">{log.details.show ? 'Open' : 'Collapsed'}</strong></span>
                        )}
                        {log.actionType === 'guidelines_opened' && (
                          <span>Opened layout documentation guidelines for current role</span>
                        )}
            ...
            {filteredLogs.length === 0 && (
              <div className="text-center p-12 text-bark-500 font-sans text-xs space-y-2">
                ...
                <p>No telemetry events match active search queries or filters.</p>
              </div>
            )}
          </div>
          ...
          <div className="p-3 bg-sand-50/10 border-t border-sand-150 text-[9.5px] text-bark-500 font-mono flex items-center justify-between">
            <span>ISO-27001 Atelier Log Integrity: Active</span>
            <span className="text-emerald-700 font-bold">Secure Telemetry Sandbox Session</span>
          </div>
