...
export default function HeroCarousel({ onExploreCatalog, onOpenSizingCalculator }) {
  ...
  const slides = [
    {
      id: 0,
      badge: 'Perfect Fit Collection No. 01',
      badgeIcon: <Compass className="w-3.5 h-3.5 text-clay-500 animate-spin-slow" />,
      title: (
        <>
          Craft Your Absolute <br />
          <span className="font-serif italic text-clay-700 font-light block mt-1">Perfect Fit Masterpiece</span>
        </>
      ),
      description: 'Curating elevated, high-fashion sewing patterns for the modern maker. Supported by professional sizing guide algorithms, detailed line diagrams, and sandbox checkout trials.',
      ctaText1: 'Browse Curated Patterns',
      ctaText2: 'Find Your Custom Size',
      bgClass: 'from-[#FAF8F5] via-[#FFFDFB] to-[#F5EFEB]',
      accentColor: 'text-clay-600',
      image: dressImg,
      modelTag: 'Aurelia Wrap Dress / Silhouette No. 8',
    },
    {
      id: 1,
      badge: 'Perfect Fit Collection No. 02',
      badgeIcon: <BookOpen className="w-3.5 h-3.5 text-clay-500" />,
      title: (
        <>
          Timeless Blueprints For <br />
          <span className="font-serif italic text-clay-700 font-light block mt-1">The Slow Fashion Maker</span>
        </>
      ),
      description: 'Every digital pattern package includes high-resolution grading files, full instruction guides, pattern layouts for US Letter/A4, copyshop options, and couture-level lining guides.',
      ctaText1: 'Explore Master Blueprints',
      ctaText2: 'Pattern Specifications',
      bgClass: 'from-[#FAF8F5] via-[#FCFAF7] to-[#F1ECE6]',
      accentColor: 'text-clay-600',
      image: trenchImg,
      modelTag: 'Perfect Fit Utility Trench / Silhouette No. 12',
    },
    {
      id: 2,
      badge: 'Perfect Fit Collection No. 03',
      badgeIcon: <Ruler className="w-3.5 h-3.5 text-clay-500" />,
      title: (
        <>
          Mathematical Precision &amp; Fit <br />
          <span className="font-serif italic text-clay-700 font-light block mt-1">Made Simple &amp; Elegant</span>
        </>
      ),
      description: 'Input your measurements to dynamically sync target sizes. We provide custom shortening and lengthening slash indicators to ensure your finished design hugs your frame with couture precision.',
      ctaText1: 'Calculate Active Size',
      ctaText2: 'Review Sizing Grid',
      bgClass: 'from-[#FFFDFB] via-[#FAF9F5] to-[#EFEAE4]',
      accentColor: 'text-clay-600',
      image: trouserImg,
      modelTag: 'Palazzo Wide-Leg Trouser / Silhouette No. 10',
    }
  ];
          ...
          return (
            ...
            >
              ...
              <div className="absolute inset-0 z-0 pointer-events-none select-none overflow-hidden" id={`hero-slide-bg-${slide.id}`}>
                <img
                  src={slide.image}
                  alt="Perfect Fit lookbook background"
              ...
              <div className="max-w-7xl mx-auto w-full relative z-10 py-12" id={`hero-slide-inner-${slide.id}`}>
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12 items-center" id="hero-slide-grid">
                  ...
                  <div className="md:col-span-8 lg:col-span-7 space-y-6 lg:space-y-8" id="hero-headline-block">
                    ...
                    {/* Description */}
                    <p className="text-bark-550 text-xs sm:text-sm md:text-base max-w-xl leading-relaxed font-sans font-medium" id="hero-subtext">
                      {slide.description}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-3.5 pt-2" id="hero-cta-buttons">
                      ...
                      <button
                        ...
                        className="bg-white/85 border border-sand-250 hover:bg-sand-50 text-bark-800 font-medium text-xs px-5 py-3 rounded-full transition-all flex items-center gap-2.5 cursor-pointer font-sans tracking-widest uppercase active:scale-[0.98]"
                        id="hero-sizing-btn"
                      >
                        {slide.ctaText2}
                        {slide.id === 0 ? (
          ...
          {slides.map((slide, idx) => (
            <button
              ...
              className={`h-1.5 transition-all duration-300 cursor-pointer ${
                currentSlide === idx ? 'w-8 bg-clay-605' : 'w-3 bg-sand-250 hover:bg-sand-350'
              }`}
              title={`Slide ${idx + 1}`}
              id={`carousel-dot-${idx}`}
            />
