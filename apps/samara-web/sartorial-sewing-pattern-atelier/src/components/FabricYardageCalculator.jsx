...
export default function FabricYardageCalculator({ selectedSize = '8', onApplyYardage }) {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('sartorial_atelier_fabric_stash');
      if (saved) {
  ...
  const activePattern = useMemo(() => {
    return SEWING_PATTERNS.find(p => p.id === selectedPatternId) || SEWING_PATTERNS[0];
  }, [selectedPatternId]);
  ...
  const filteredPatterns = useMemo(() => {
    if (!searchTerm.trim()) return SEWING_PATTERNS.slice(0, 12);
    return SEWING_PATTERNS.filter(p =>
      p.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      p.category.toLowerCase().includes(searchTerm.toLowerCase())
    ).slice(0, 12);
  }, [searchTerm]);

  const sizeNum = parseInt(localSize) || 8;
...
Estimated Nesting Efficiency: ${calculations?.efficiencyScore}%`;
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full justify-between" id="calculator-sidebar-inner">
      <div className="space-y-5 flex-1 overflow-y-auto p-4 sm:p-5 scrollbar-thin">
        ...
        <div className="bg-sand-50/65 border border-sand-200/50 rounded p-3 space-y-1 text-xs">
          ...
          <div className="text-[10px] text-bark-500 font-mono flex flex-wrap gap-x-2.5 gap-y-0.5">
            ...
            <span>Complexity: <span className="font-bold text-clay-650">{layoutComplexity.toUpperCase()}</span></span>
          </div>
        </div>
        ...
        <div className="text-center bg-white border border-sand-200/80 rounded-[4px] p-4.5 shadow-3xs relative overflow-hidden" id="sidebar-calculation-hero">
          ...
          <span className="text-[8.5px] font-mono font-bold text-clay-700 uppercase tracking-widest block mb-1">
            Required Yardage
          </span>
          ...
          <p className="text-[9px] text-bark-450 mt-1.5 leading-tight">
            Dynamic estimate factoring seam allowance & grain layout buffers.
          </p>
        </div>

        {/* Breakdown summary cards */}
        <div className="space-y-2.5" id="sidebar-supplements-summary">
          <h4 className="text-[9px] font-mono font-bold text-bark-400 uppercase tracking-widest">
          ...
          <div className="bg-white border border-sand-150 rounded-[4px] divide-y divide-sand-100 text-xs font-medium font-sans">
            ...
            <div className="flex justify-between items-center p-2.5">
              ...
              {hasLining ? (
                ...
                <span className="text-[10px] text-bark-400 italic font-normal">Not included</span>
              )}
            </div>
      ...
      <div className="border-t border-sand-200/60 p-4 space-y-2 bg-sand-50/40">
        ...
        >
          <ClipboardCopy className="w-3.5 h-3.5 text-bark-600" />
          Copy Spec Sheet
        </button>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-sand-200/60 pb-4">
        <div className="space-y-0.5">
          <h3 className="text-sm font-serif font-semibold text-bark-900 uppercase tracking-widest flex items-center gap-2">
            <Scissors className="w-4 h-4 text-clay-605" /> Pattern Yardage & Bolt Calculator
          </h3>
          <p className="text-[10px] text-bark-500 font-medium">
            Calculate precise linear shell material, lining & interfacing needs mapped to size & grain alignments.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2" id="header-action-toggles">
          ...
          <button
            ...
            title="Open Interactive Yardage Sidebar"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" id="fabric-calculator-grid">
        ...
        <div className="lg:col-span-7 space-y-5" id="params-column">
          ...
          <div className="space-y-1.5 relative">
            <label className="text-[10px] font-mono font-bold text-bark-500 uppercase tracking-widest block">
              1. Choose Specific Sewing Pattern
            </label>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Filter sewing patterns..."
                  value={searchTerm}
              ...
              >
                ...
                <div className="space-y-1 flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[8px] font-mono uppercase text-[#e0a894] font-semibold bg-bark-900/5 px-1 rounded">
                      Standard Grade
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    <span className="text-[8.5px] text-bark-600 font-sans">
                      <strong>Fabric Fibers:</strong> {activePattern.fabricSuggestions?.slice(0, 3).join(', ')}
                    </span>
                  </div>
                </div>
              </motion.div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-[10px] font-mono font-bold text-bark-500 uppercase tracking-widest">
                2. Fabric Bolt Width
              </label>
              <button
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-[10px] font-mono font-bold text-bark-500 uppercase tracking-widest">
                3. Design Layout Complexity
              </label>
              <HelpCircle className="w-3.5 h-3.5 text-bark-400 cursor-pointer" title="Adjust fabric requirement calculations based on piece shape complexity and cut directions." />
            </div>
          ...
          <div className="border-t border-sand-150 pt-3 flex flex-wrap gap-5">
            <label className="flex items-center gap-2.5 cursor-pointer text-xs font-sans text-bark-800 select-none">
              ...
              />
              <div className="leading-tight">
                <span className="font-bold block">Include Lining Fabric</span>
                <span className="text-[9px] text-bark-450 font-medium font-mono">Estimates backing silk / acetate yards</span>
              </div>
            </label>
            ...
            <label className="flex items-center gap-2.5 cursor-pointer text-xs font-sans text-bark-800 select-none">
              ...
              />
              <div className="leading-tight">
                <span className="font-bold block">Include Fusible Interfacing</span>
                <span className="text-[9px] text-bark-450 font-medium font-mono">Estimates adhesive tailoring structure canvas</span>
              </div>
            </label>
          </div>
        ...
        <div className="lg:col-span-5 bg-[#FAF8F5] border border-sand-200/80 rounded-[4px] p-4.5 flex flex-col justify-between shadow-xs relative" id="outcomes-column">
          ...
          <div className="flex items-center justify-between pb-2 border-b border-sand-200/60 mb-2">
            <span className="text-[9px] font-mono font-bold text-bark-500 uppercase tracking-widest flex items-center gap-1">
              <Layers className="w-3 h-3 text-clay-605" /> Live Result Sidebar
            </span>
            <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" title="Connected to design canvas" />
          </div>
        ...
        {isSidebarOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 bg-bark-950 z-40 cursor-pointer"
              id="sidebar-backdrop"
            />
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 h-full w-full max-w-sm sm:max-w-md bg-white border-l border-sand-200 shadow-2xl z-50 flex flex-col justify-between"
            ...
            >
              ...
              <div className="border-b border-sand-200 p-4 sm:p-5 flex items-center justify-between bg-[#FAF8F5]" id="sidebar-drawer-header">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-xs font-serif font-bold uppercase tracking-widest text-bark-900">
                      Bespoke Yardage Sidebar
                    </h3>
                    <p className="text-[9px] text-bark-500 font-sans font-medium">
                      Live calculations pinned to your fitting room
                    </p>
                  </div>
                </div>
              ...
              <div className="border-b border-sand-100 bg-[#FAF8F5]/40 p-4 space-y-3" id="sidebar-drawer-sliders">
                <span className="text-[8.5px] font-mono font-bold text-bark-450 uppercase tracking-widest block">
